<div class="px-6 py-10 lg:px-12 max-w-7xl mx-left space-y-12">
    {{-- Refined Professional Header --}}
    <div
        class="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-10 border-b border-slate-100 dark:border-slate-800">
        <div class="space-y-2">
            <h1 class="text-3xl lg:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">
                Teacher HRM Dashboard
            </h1>
            <p class="text-slate-500 dark:text-slate-400 font-medium max-w-2xl leading-relaxed">
                Centralized human resource management for education professionals. Monitor service records, career
                milestones, and administrative operations.
            </p>
        </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 lg:gap-8">
        ...
        @foreach($items as $item)
            ...
            <a href="{{ $item['route'] ?? '#' }}" wire:navigate
                class="group relative flex items-start gap-8 p-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-[2.5rem] transition-all duration-300 hover:shadow-2xl {{ $item['hover_shadow'] }} {{ $item['hover_border'] }} dark:hover:border-slate-800">
                ...
                <div class="flex-1 space-y-2 py-1">
                    ...
                    <p
                        class="text-[14px] font-semibold text-slate-500 dark:text-slate-400 leading-snug group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors">
                        {{ $item['desc'] }}
                    </p>
