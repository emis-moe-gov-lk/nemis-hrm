    ...
    <div class="relative mb-6 w-full">
        ...
        <div class="max-w-2xl my-12">
            <div class="relative flex justify-between w-full px-2">
                ...
                <div class="absolute left-12 right-12 top-5 -translate-y-1/2 h-0.5 -z-10">
                    <!-- Background Line -->
                    <div class="w-full h-full bg-gray-200 dark:bg-neutral-800 rounded-full opacity-60"></div>
                    <!-- Filled Progress Line -->
                    <div class="absolute top-0 left-0 h-full bg-indigo-600 dark:bg-indigo-500 rounded-full transition-all duration-500 ease-out"
                        style="width: {{ $maxStep > 1 ? (($step - 1) / ($maxStep - 1)) * 100 : 0 }}%"></div>
                </div>
...
<form wire:submit.prevent="save" class="mt-6 max-w-2xl space-y-6">
        ...
        <div class="mb-6 p-4 rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-700/30 text-amber-900 dark:text-amber-100 shadow-sm">
            <div class="flex items-start gap-4">
                ...
                <div class="space-y-2 pt-1">
                    ...
                    <p class="text-sm leading-relaxed text-amber-800 dark:text-amber-300 opacity-90">
                        பதிவைத் தொடங்குவதற்கு முன்னர், உங்கள் தேசிய அடையாள அட்டை எண் ஏற்கனவே கணினி முறைமையில் உள்ளதா என்பதை உறுதிப்படுத்திக் கொள்ளுங்கள். நீங்கள் உள்ளிடும் NIC எண் சரியானது என்பதை உறுதிப்படுத்தியவுடன், தொடரவும்.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:field>
            <flux:select label="Teacher appointment category" wire:model.live.debounce.150ms="teacherCategory">
                <option value="">Select</option>
        ...
        <flux:field>
            <flux:select label="Types of teachers" wire:model.live.debounce.150ms="teacherType">
                <option value="">Select</option>
        ...
        <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Appointment Subject" wire:model.live.debounce.150ms="appointmentSubject">
                        <option value="">Select</option>
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Appointment medium" wire:model.live.debounce.150ms="appointmentMedium">
                        <option value="">Select</option>
        ...
        <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Main/Most teaching subject" wire:model.live.debounce.150ms="mainTeachingSubject">
                        <option value="">Select</option>
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Secondary teaching subject(Optional)"
                        wire:model.live.debounce.150ms="secondaryTeachingSubject">
                        <option value="">Select</option>
                        @foreach ($subjectOption as $subject)
                        <option value="{{ $subject->subject_id }}">{{ $subject->name_en }}</option>
                        @endforeach
        ...
        <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Zonal Education Office" wire:model.live.debounce.150ms="zonalEducationOffice">
                        <option value="">Select</option>
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Institution Category" wire:model.live.debounce.150ms="institutionCategory">
                        <option value="">Select</option>
        ...
        <flux:field>
            <flux:select label="First Appointment Institution" wire:model.live.debounce.150ms="institution">
                <option value="">Select</option>
                @foreach ($institutionOption as $institution)
                <option value="{{ $institution->workplace_id }}">
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:radio.group label="Select registration type for the Teacher"
            wire:model.live.debounce.150ms="teacherRegType">
            <flux:radio name="teacherRegType" value="new" disabled label="New teacher"
        ...
        <div class=" border border-red-500 bg-red-100 rounded-lg p-2">
            <p class=" text-red font">නවක ගුරුවරයෙකු ලෙස පත්වීමක් ලබා නොගන්නා අවස්තාවක, සේවා කාලයක්
                සහිත ගුරුවරයෙකු ලියාපදිංචි කිරීම සඳහා පමණි
            <div class="br"></div>Only for the registration of a teacher with a period of service,
            in the event that an appointment is not obtained as a new teacher.</p>
        </div>
        <div class=" space-y-6">
            <div class="flex flex-col md:flex-row gap-4">
            ...
            <flux:field>
                <flux:select label="Current teaching subject"
                    wire:model.live.debounce.150ms="currentTeachingSubject">
                    <option value="">Select</option>
                    @foreach ($subjectOption as $subject)
                    <option value="{{ $subject->subject_id }}">{{ $subject->name_en }}</option>
                    @endforeach
                ...
                <div class="md:w-1/2 w-full">
                    <flux:field>
                        <flux:select label="Zonal Education Office"
                            wire:model.live.debounce.150ms="currentZonalEducationOffice">
                            <option value="">Select</option>
                ...
                <div class="md:w-1/2 w-full">
                    <flux:field>
                        <flux:select label="Institution Category"
                            wire:model.live.debounce.150ms="currentInstitutionCategory">
                            <option value="">Select</option>
                            @foreach ($institutionCategoryOption as $data)
                            <option value="{{ $data->institution_category_id }}">
                                {{ $data->institution_category_name }}
            ...
            <flux:field>
                <flux:select label="Current Appointment Institution"
                    wire:model.live.debounce.150ms="currentInstitution">
                    <option value="">Select</option>
            ...
            <flux:field>
                <flux:select label="Current Appointmented Position" wire:model.live.debounce.150ms="currentPosition">
                    <option value="">Select</option>
                    @foreach ($positionOption as $position)
                    <option value="{{ $position->position_id }}">
                        {{ $position->position_name }}
