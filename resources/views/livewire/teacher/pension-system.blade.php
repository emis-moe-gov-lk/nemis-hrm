<div class="min-h-screen bg-slate-50/50 p-4 antialiased">
    ...
    <header class="max-w-7xl mx-left mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-black text-slate-900 tracking-tight">Pension, Retirement & Resignation Management
            </h1>
            <p class="text-slate-500 text-sm">Review and authorize teacher pension, retirement and resignation.</p>
        </div>
    </header>
            ...
            <div class="promotion-row group bg-white rounded-2xl border border-slate-200 p-4 shadow-sm hover:border-indigo-300 transition-all flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-6"
                ...
                <div class="flex items-center gap-3 lg:w-1/4 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                    <div class="flex-1 text-center">
                        <p class="text-[9px] font-black text-slate-400 uppercase">Proposed status</p>
                        <span class="text-[11px] font-bold text-slate-600">
                            {{ $employee->date_of_birth
            ? (\Carbon\Carbon::parse($employee->date_of_birth)->age >= 55
                ? 'Pension'
                : 'Resigned or Retired')
            : 'N/A' }}
                        </span>
                ...
                <div class="flex items-center justify-between lg:justify-end gap-3 lg:w-1/4 lg:ml-auto">
                    <div class="hidden xl:block text-right mr-4">
                        <p class="text-[9px] font-black text-slate-300 uppercase">Age</p>
                        <p class="text-xs font-bold text-slate-700">
                            {{ $employee->date_of_birth
            ? \Carbon\Carbon::parse($employee->date_of_birth)->diff(\Carbon\Carbon::now())->y . ' Years ' .
            \Carbon\Carbon::parse($employee->date_of_birth)->diff(\Carbon\Carbon::now())->m . ' Months'
            : 'N/A' }}
                        </p>
                    </div>
                    ...
                    <div class="flex gap-2 w-full lg:w-auto">
                        @if (\Carbon\Carbon::parse($employee->date_of_birth)->age >= 55)
                            {{-- Pension Button: Emerald Green & Retirement Icon --}}
                            <button wire:click="pensionTeacher({{ $employee->id }})"
                                class="flex-1 lg:flex-none px-5 py-2.5 bg-emerald-600 text-white rounded-xl text-xs font-bold shadow-md shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-95 group/btn">
                                <i
                                    class="fa-solid fa-house-user text-[10px] group-hover/btn:-translate-y-0.5 transition-transform mr-1"></i>
                                <span class="text-[10px] font-bold uppercase tracking-tight">Pension</span>
                            </button>
                        @else
                            {{-- Resigned Button: Rose Red & Exit Icon --}}
                            <button wire:click="pensionTeacher({{ $employee->id }})"
                                class="flex-1 lg:flex-none px-5 py-2.5 bg-rose-600 text-white rounded-xl text-xs font-bold shadow-md shadow-rose-100 hover:bg-rose-700 transition-all active:scale-95 group/btn">
                                <i
                                    class="fa-solid fa-arrow-right-from-bracket text-[10px] group-hover/btn:-translate-y-0.5 transition-transform mr-1"></i>
                                <span class="text-[10px] font-bold uppercase tracking-tight">Resigned</span>
                            </button>
                        @endif
                    </div>
    ...
    <flux:modal wire:model="showModalPensionTeacher" name="pension-teacher" class="p-0 overflow-hidden !max-w-2xl">
        <div class="px-8 pt-8 pb-6 text-center">
            <flux:heading size="xl" class="text-slate-900 font-bold tracking-tight">Teacher Pension</flux:heading>
            <flux:subheading class="text-slate-500">Official Pension for the teachers
            </flux:subheading>
        </div>

        <form wire:submit.prevent="updateTeacherPension" class="px-8 pb-8 space-y-8">
            ...
            <div class="bg-slate-50 rounded-2xl p-5 border border-slate-100 flex items-center gap-4">
                ...
                <div class="text-right">
                    <p class="text-[10px] uppercase font-bold tracking-widest text-slate-400">Age</p>
                    @php
                        $dob = $teacher?->date_of_birth ? \Carbon\Carbon::parse($teacher->date_of_birth) : null;
                        $diff = $dob ? $dob->diff(now()) : null;
                    @endphp

                    <p class="text-sm font-bold text-slate-700">
                        {{ $dob ? $diff->y . ' Years ' . $diff->m . ' Months' : 'N/A' }}
                    </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <flux:field>
                        <flux:label class="text-[11px] uppercase font-bold tracking-wider text-slate-500">Service
                            Termination Reason</flux:label>
                        <flux:select wire:model.live="pension_reason" placeholder="Select Termination Reason">
                            @foreach ($reasons as $reason)
                                <flux:select.option value="{{ $reason->termination_id }}">{{ $reason->reason }}
                                </flux:select.option>
                            @endforeach
                        </flux:select>
                    </flux:field>
                    ...
                    <flux:field>
                        ...
                        <flux:input type="date" wire:model.live="pension_effect_date" class="!bg-white" />
                    </flux:field>
                </div>
            ...
            <div class="flex items-center justify-between px-8 py-6 bg-slate-50 border-t border-slate-100">
                ...
                <flux:button type="submit"
                    ...
                    <span class="text-xs font-bold uppercase tracking-widest">Complete Pension</span>
                </flux:button>
            </div>
        </form>
