    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            <flux:heading size="xl" class="text-3xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-blue-500">
                Sri Lanka Teacher Educator Service Directory
            </flux:heading>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <flux:modal.trigger name="search-profile">
                <flux:button variant="subtle" icon="magnifying-glass" class="h-11 !bg-white dark:!bg-slate-900 shadow-sm">Search Teacher Educator Officers...</flux:button>
            </flux:modal.trigger>
        ...
        @forelse($employees as $employee)
            <div class="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 transition-all duration-300 hover:shadow-xl hover:border-indigo-200 flex flex-col lg:flex-row lg:items-center gap-6">
                ...
                <div class="flex items-center gap-2 lg:ml-auto">
                    <flux:button href="{{ route('sltes.profile.index', $employee->id) }}" size="sm" class="!rounded-xl !bg-slate-900 !text-white font-bold px-4">View</flux:button>
                    <flux:dropdown>
                ...
                @forelse($results as $sltes)
                    <a href="{{ route('sltes.profile.index', $sltes->id) }}" class="flex items-center gap-4 p-4 rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-900/10 border border-transparent hover:border-indigo-100 transition-all">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-indigo-400">{{ $sltes->currentAppointment?->workplace?->office()?->name }}</p>
                        </div>
                    </a>
                @empty
