<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Create Teacher Educator Officer') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Create Teacher Educator Officer profile and account') }}</flux:subheading>
...
<form wire:submit.prevent="save" class="mt-6 max-w-2xl space-y-6">
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:radio.group label="Select registration type for the Teacher Educator Officer"
            wire:model.live.debounce.150ms="sltesRegType">
            <flux:radio name="sltesRegType" value="new" disabled label="New Teacher Educator Officer"
                description="New Teacher Educator Officer users can perform any action." />
            <flux:radio name="sltesRegType" value="existing" label="Existing Teacher Educator Officer"
                description="Existing Teacher Educator Officer users have the ability to read, create, and update." />
        </flux:radio.group>

        @if ($sltesRegType != 'new')
        <div class=" border border-red-500 bg-red-100 rounded-lg p-2">
            <p class=" text-red font">නවක ගුරු අධ්‍යාපනඥ නිළදාරියකු ලෙස පත්වීමක් ලබා නොගන්නා අවස්තාවක, සේවා කාලයක්
                සහිත ගුරු අධ්‍යාපනඥ නිළදාරියකු ලියාපදිංචි කිරීම සඳහා පමණි
            <div class="br"></div>Only for the registration of a Teacher Educator Officer with a period of service,
            in the event that an appointment is not obtained as a new Teacher Educator Officer.</p>
        </div>
        <div class=" space-y-6">
            <div class="flex flex-col md:flex-row gap-4">
    ...
    <div class="flex justify-between mt-8 bg-gray-50 dark:bg-neutral-900 p-2 rounded-xl border border-gray-200 dark:border-neutral-700">
        ...
        @if ($step < $maxStep)
            ...
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">
                <span wire:loading.remove>Create Teacher Educator Officer</span>
