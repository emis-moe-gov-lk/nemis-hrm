<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Create Principal') }}</flux:heading>
...
<form wire:submit.prevent="save" class="mt-6 max-w-2xl space-y-6">
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:field>
            <flux:select label="Principal recruitment category" wire:model.live.debounce.150ms="principalCategory">
                <option value="">Select</option>
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:radio.group label="Select registration type for the Principal"
            wire:model.live.debounce.150ms="principalRegType">
            <flux:radio name="principalRegType" value="new" disabled label="New principal"
                description="New principal users can perform any action." />
            <flux:radio name="principalRegType" value="existing" label="Existing principal"
                description="Existing principal users have the ability to read, create, and update." />
        </flux:radio.group>

        @if ($principalRegType != 'new')
        <div class=" border border-red-500 bg-red-100 rounded-lg p-2">
            <p class=" text-red font">නවක විදුහල්පතිවරයකු ලෙස පත්වීමක් ලබා නොගන්නා අවස්තාවක, සේවා කාලයක්
                සහිත විදුහල්පතිවරයකු ලියාපදිංචි කිරීම සඳහා පමණි
            <div class="br"></div>Only for the registration of a principal with a period of service,
            in the event that an appointment is not obtained as a new principal.</p>
        </div>
        <div class=" space-y-6">
            <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <div class="md:w-1/2 w-full">
                    <flux:field>
                        <flux:select label="Cadre medium"
                            wire:model.live.debounce.150ms="cadreMedium">
                            <option value="">Select</option>
                            @foreach ($mediumOption as $medium)
                            <option value="{{ $medium->medium_id }}">{{ $medium->name }}
                            </option>
                            @endforeach
                        </flux:select>
                    </flux:field>
                ...
                <div class="md:w-1/2 w-full">
                    <flux:field>
                        <flux:select label="Cadre subject"
                            wire:model.live.debounce.150ms="cadreSubject">
                            <option value="">Select</option>
    ...
    <div class="flex justify-between mt-8 bg-gray-50 dark:bg-neutral-900 p-2 rounded-xl border border-gray-200 dark:border-neutral-700">
        ...
        @if ($step < $maxStep)
            ...
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">
                <span wire:loading.remove>Create Principal</span>
