<div class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-sm font-medium text-slate-500">{{ $policy->circular_number }}</span>
            </div>
            <flux:heading size="xl" level="1">{{ $policy->title }}</flux:heading>
            <flux:subheading size="lg">{{ __('Detailed summary of transfer policy rules and approval workflow.') }}</flux:subheading>
        </div>

        <div class="flex items-center gap-3">
            <flux:button href="{{ route('transfer.transfer-policies') }}" variant="ghost" icon="chevron-left">{{ __('Back to List') }}</flux:button>
            @if(!$policy->is_locked)
            <flux:button href="{{ route('transfer.transfer-policy.edit', $policy->policy_id) }}" variant="ghost" icon="pencil-square">{{ __('Edit Policy') }}</flux:button>
            @endif

            <flux:button wire:click="toggleLock" wire:confirm="{{ $policy->is_locked ? __('Are you sure you want to unlock this policy?') : __('Are you sure you want to lock this policy? Locked policies cannot be edited or deleted.') }}" variant="ghost" color="{{ $policy->is_locked ? 'orange' : 'slate' }}" icon="{{ $policy->is_locked ? 'lock-closed' : 'lock-open' }}">{{ $policy->is_locked ? __('Unlock') : __('Lock') }}</flux:button>

            @if(!$policy->is_locked)
            <flux:button wire:click="deletePolicy" wire:confirm="{{ __('Are you sure you want to delete this policy?') }}" variant="ghost" color="red" icon="trash">{{ __('Delete') }}</flux:button>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-zinc-800 p-8 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 text-sm">
                    <div class="space-y-4">
                        <div>
                            <p class="text-slate-500 dark:text-zinc-400 mb-1 font-medium">{{ __('Applicable Service') }}</p>
                            <p class="font-bold text-slate-900 dark:text-zinc-100">{{ $policy->service->service_name ?? 'N/A' }}</p>
                        </div>
                        <div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="font-bold text-slate-900 dark:text-zinc-100">{{ $policy->authority->name ?? 'N/A' }}</p>
                        </div>
                        <div>
                ...
                <div class="border-t border-slate-100 dark:border-zinc-800 mt-8 pt-6">
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-zinc-800 p-8 shadow-sm">
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 border border-slate-100 dark:border-zinc-800 rounded-2xl bg-slate-50/50 dark:bg-zinc-800/30">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-bold text-slate-900 dark:text-zinc-100">{{ $category->transfer_category_name }}</span>
                    ...
                    @empty
                    <div class="py-12 border-2 border-dashed border-slate-100 dark:border-zinc-800 rounded-2xl flex flex-col items-center justify-center text-slate-400">
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-zinc-800 p-8 shadow-sm">
                ...
                <div class="space-y-4">
                    @forelse($policy->scoreRules->where('active_status', true) as $scoreRule)
                        <div class="rounded-2xl border border-slate-100 bg-slate-50/60 p-4 dark:border-zinc-800 dark:bg-zinc-800/30">
        ...
        <div>
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-zinc-800 p-8 shadow-sm h-full">
