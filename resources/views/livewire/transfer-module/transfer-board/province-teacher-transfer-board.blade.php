<div class="p-6 lg:p-8 space-y-8">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @if($currentWorkplace && !$selectedBoard && !$showCreateBoardFlow)
    ...
    @if(!$currentWorkplace)
        <div class="rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
            <div class="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-slate-100 dark:bg-zinc-800">
                <flux:icon name="shield-exclamation" class="h-10 w-10 text-slate-400 dark:text-zinc-500" />
            </div>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
                        @forelse($boards as $board)
                            ...
                            <tr class="transition-all duration-200 hover:bg-slate-50/50 dark:hover:bg-zinc-800/20">
                                ...
                                <td class="px-4 py-5 align-top">
                                    <div class="min-w-0 space-y-1">
                                        <p class="break-words text-sm font-semibold leading-5 text-slate-800 dark:text-zinc-200">{{ $board->category?->transfer_category_name ?? __('N/A') }}</p>
                                ...
                                <td class="px-4 py-5 align-top">
                                    <div class="min-w-0 space-y-2">
                                        <div>
                                            <p class="text-[11px] font-bold uppercase tracking-[0.22em] text-slate-400 dark:text-zinc-500">{{ __('Chairman') }}</p>
                                            <p class="break-words text-sm font-semibold text-slate-800 dark:text-zinc-200">{{ $board->chairman?->full_name ?? __('N/A') }}</p>
                                        </div>

                                        <div>
                                            <p class="text-[11px] font-bold uppercase tracking-[0.22em] text-slate-400 dark:text-zinc-500">{{ __('Secretary') }}</p>
    ...
    @elseif($showCreateBoardFlow)
        @include('livewire.transfer-module.transfer-board.partials.province-create-board-flow')
    @else
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-4">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <p class="mt-2 text-sm text-slate-500 dark:text-zinc-400">{{ $selectedBoard->category?->transfer_category_name ?? __('N/A') }}</p>
        ...
        @if(!$isAppealBoard)
            @include('livewire.transfer-module.transfer-board.partials.board-school-balance-panel')
        @endif
                        ...
                        @if(($applications?->count() ?? 0) > 0)
                            @foreach($applications as $application)
                                @include($isAppealBoard
                                    ? 'livewire.transfer-module.transfer-board.partials.province-appeal-board-application-row'
                                    : 'livewire.transfer-module.transfer-board.partials.province-transfer-board-application-row')
                            @endforeach
                        @else
                            <tr>
                                <td colspan="6" class="px-6 py-20 text-center">
        ...
        <div class="space-y-8">
            ...
            @if($attendanceBoard)
                ...
                <div class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                    <div class="border-b border-slate-100 bg-slate-50/70 px-5 py-4 dark:border-zinc-800 dark:bg-zinc-800/30">
                            ...
                            <div class="grid grid-cols-1 gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_180px_180px_minmax(220px,0.9fr)] lg:items-start">
                                ...
                                <div>
                                    @if($attendanceReadOnly)
                                        ...
                                        <p class="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 text-sm font-semibold text-slate-700 dark:border-zinc-800 dark:bg-zinc-800/30 dark:text-zinc-200">
                                ...
                                <div>
                                    @if($attendanceReadOnly)
                                        ...
                                        <p class="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 text-sm text-slate-600 dark:border-zinc-800 dark:bg-zinc-800/30 dark:text-zinc-300">
