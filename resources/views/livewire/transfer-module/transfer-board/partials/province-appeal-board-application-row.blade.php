...
<tr class="transition-all duration-200 hover:bg-slate-50/50 dark:hover:bg-zinc-800/20">
    <td class="px-6 py-5 align-top">
        <div class="space-y-2">
            <div>
                <p class="text-sm font-black text-slate-900 dark:text-white">{{ $appeal->appeal_id }}</p>
                <p class="text-xs text-slate-500 dark:text-zinc-400">
                    {{ __('Appeal #:number | Application :application', ['number' => $appeal->number_of_appeals, 'application' => $appeal->transfer_application_id]) }}
                </p>
                <p class="text-xs text-slate-500 dark:text-zinc-400">{{ $appeal->created_at?->format('Y-M-d') }}</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                <div class="flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-slate-100 text-xs font-bold text-slate-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300">
                    {{ \Illuminate\Support\Str::substr($transferApplication?->employee?->full_name ?? 'T', 0, 1) }}
                </div>

                <div>
                    <p class="text-sm font-semibold text-slate-800 dark:text-zinc-200">{{ $transferApplication?->employee?->full_name ?? __('Teacher') }}</p>
                    <p class="text-xs text-slate-500 dark:text-zinc-400">{{ $transferApplication?->employee?->nic ?? __('NIC unavailable') }}</p>
                </div>
            </div>
        </div>
    </td>
    ...
    <td class="px-6 py-5 align-top">
        <div class="flex flex-wrap justify-end gap-2">
            ...
            <flux:button
                href="{{ route('transfer-board.teacher-profile-for-appeal-board', ['id' => $appeal->appeal_id, 'board' => $boardRouteScope, 'selectedBoardId' => $selectedBoard->board_id]) }}"
                variant="{{ $appeal->appeal_status === 'pending' ? 'primary' : 'ghost' }}"
                icon="{{ $appeal->appeal_status === 'pending' ? 'check-circle' : 'pencil-square' }}"
                size="sm">
