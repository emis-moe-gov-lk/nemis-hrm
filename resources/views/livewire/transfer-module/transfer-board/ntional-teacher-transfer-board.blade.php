<div class="p-6 lg:p-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
            <flux:heading size="xl" level="1">{{ __('National Teacher Transfer Board') }}</flux:heading>
            <flux:subheading size="lg">{{ __('Oversee and manage teacher transfer applications for national schools') }}</flux:subheading>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200 dark:border-zinc-800 shadow-sm overflow-hidden">
        {{-- Filters Bar --}}
        <div class="p-6 bg-slate-50/50 dark:bg-zinc-800/30 border-b border-slate-100 dark:border-zinc-800">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition-all duration-200">
                        <td class="px-6 py-4">
                            ...
                            <span class="block text-xs text-slate-400 dark:text-zinc-500">{{ $app->created_at->format('Y-M-d') }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center border border-slate-200 dark:border-zinc-700">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-1">
                                ...
                                <span class="text-xs font-medium text-slate-600 dark:text-zinc-400">{{ $app->category->transfer_category_name ?? 'N/A' }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="block text-sm font-medium text-slate-700 dark:text-zinc-300 truncate max-w-[220px]" title="{{ $app->currentWorkplace->office_name ?? 'N/A' }}">
                            ...
                            <span class="text-[10px] text-blue-500 dark:text-blue-400 font-bold uppercase tracking-wider mt-1 flex items-center gap-1">
                                ...
                                {{ __('Requested a National School') }}
                            </span>
                            @else
                            <span class="text-[10px] text-amber-500 dark:text-amber-400 font-bold uppercase tracking-wider mt-1 flex items-center gap-1">
                                ...
                                {{ __('Currently in National School') }}
                            </span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-2">
                                <flux:button href="{{ route('transfer-board.teacher-profile-for-transfer-board', ['id' => $app->transfer_application_id, 'board' => 'national']) }}" variant="ghost" icon="eye" size="sm" />
                        ...
                        <td colspan="6" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-slate-500 dark:text-zinc-400 max-w-xs mx-auto">
                                    {{ __('We couldn\'t find any national-school transfer applications for the current criteria.') }}
                                </p>
                            </div>
                        </td>
                    </tr>
        ...
        {{-- Pagination --}}
        <div class="p-6 bg-slate-50 dark:bg-zinc-800/30 border-t border-slate-100 dark:border-zinc-800">
            {{ $applications->links() }}
        </div>
    </div>
</div>