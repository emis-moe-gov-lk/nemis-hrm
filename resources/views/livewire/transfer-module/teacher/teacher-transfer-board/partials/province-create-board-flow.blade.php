    ...
    <div class="rounded-3xl border border-slate-300 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        <div class="border-b border-slate-200 bg-slate-50/70 p-6 dark:border-zinc-700 dark:bg-zinc-800/30">
            <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                ...
                <div class="rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 gap-3 lg:grid-cols-4">
                @foreach($createBoardSteps as $stepNumber => $step)
                    ...
                    <div class="rounded-2xl border px-4 py-4 transition-all {{ $isActive ? 'border-blue-200 bg-blue-50/70 dark:border-blue-900/60 dark:bg-blue-950/20' : ($isCompleted ? 'border-emerald-200 bg-emerald-50/70 dark:border-emerald-900/60 dark:bg-emerald-950/20' : 'border-slate-300 bg-slate-50/60 dark:border-zinc-700 dark:bg-zinc-800/30') }}">
            ...
            @if($createBoardStep === 1)
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 lg:grid-cols-4">
                        ...
                        <flux:field>
                            <flux:select label="{{ __('Category') }}" wire:model.live="createTransferSubCategoryId" :invalid="$errors->has('createTransferSubCategoryId')">
                        ...
                        <flux:field>
                            <flux:input label="{{ __('Board Stage') }}" value="{{ $selectedCreateBoardStageLabel }}" readonly />
                        </flux:field>
                    </div>
                    ...
                    @error('createTransferSubCategoryId') <p class="text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p> @enderror
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-5">
                        ...
                        <div class="rounded-2xl border border-slate-300 bg-slate-50/70 p-4 dark:border-zinc-700 dark:bg-zinc-800/30">
                            ...
                            <p class="mt-2 text-base font-semibold text-slate-900 dark:text-zinc-100">{{ $selectedCreateSubCategory?->name ?? __('Select a category') }}</p>
                        </div>

                        <div class="rounded-2xl border border-slate-300 bg-slate-50/70 p-4 dark:border-zinc-700 dark:bg-zinc-800/30">
                            <p class="text-xs font-bold uppercase tracking-[0.22em] text-slate-500 dark:text-zinc-400">{{ __('Board Stage') }}</p>
                            <p class="mt-2 text-base font-semibold text-slate-900 dark:text-zinc-100">{{ $selectedCreateBoardStageLabel }}</p>
                        </div>
                    </div>
                </div>
            @elseif($createBoardStep === 2)
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-end">
                        <flux:field>
                            <flux:input
                                label="{{ __('Search Subjects') }}"
                                wire:model.live.debounce.300ms="subjectSearch"
                                placeholder="{{ __('Type a subject name...') }}"
                            />
                        </flux:field>

                        <p class="text-sm text-slate-500 dark:text-zinc-400">
                            {{ trans_choice(':count subject visible|:count subjects visible', $availableSubjects->count(), ['count' => $availableSubjects->count()]) }}
                        </p>
                    </div>

                    <div class="grid max-h-80 grid-cols-1 gap-3 overflow-y-auto rounded-3xl border border-slate-300 bg-slate-50/70 p-4 md:grid-cols-2 xl:grid-cols-3 dark:border-zinc-700 dark:bg-zinc-800/30">
                        @forelse($availableSubjects as $subject)
                            <label wire:key="create-board-subject-{{ $subject->subject_id }}" class="flex items-center gap-3 rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition hover:border-slate-300 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:border-zinc-500">
                                <input
                                    type="checkbox"
                                    wire:model.live="selectedSubjectIds"
                                    value="{{ $subject->subject_id }}"
                                    class="h-5 w-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 dark:border-zinc-600 dark:bg-zinc-900 dark:text-indigo-400 dark:focus:ring-indigo-400"
                                />
                                <span>{{ $subject->name_en }}</span>
                            </label>
                        @empty
                            <div class="rounded-2xl border border-dashed border-slate-300 bg-white/70 px-4 py-6 text-sm text-slate-500 dark:border-zinc-700 dark:bg-zinc-900/60 dark:text-zinc-400 md:col-span-2 xl:col-span-3">
                                {{ __('No subjects matched your search.') }}
                            </div>
                        @endforelse
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
                        <div class="rounded-3xl border border-slate-300 bg-slate-50/70 p-5 dark:border-zinc-700 dark:bg-zinc-800/30">
                            ...
                            <div class="mt-4 space-y-3 text-sm text-slate-700 dark:text-zinc-300">
                                ...
                                <p><span class="font-bold text-slate-500 dark:text-zinc-400">{{ __('Category') }}:</span> {{ $selectedCreateSubCategory?->name ?? __('Not selected') }}</p>
                                <p><span class="font-bold text-slate-500 dark:text-zinc-400">{{ __('Board Stage') }}:</span> {{ $selectedCreateBoardStageLabel }}</p>
