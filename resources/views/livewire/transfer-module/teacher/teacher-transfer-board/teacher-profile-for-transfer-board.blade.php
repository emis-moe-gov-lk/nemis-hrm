<div class="p-6 lg:p-8">
    @php
        ...
        $currentBoardDecision = collect($application->boardRecommendations ?? [])
            ->firstWhere('transfer_board_id', $this->selectedTeacherTransferBoard?->board_id)
    ...
    <div class="flex flex-col md:flex-row md:items-stretch justify-between mb-8 gap-4">
        ...
        <div class="flex flex-col items-start gap-4 md:min-w-[18rem] md:self-stretch md:items-end md:justify-between md:pb-2">
            ...
            @if($this->selectedTeacherTransferBoard?->isClosed())
                <flux:button icon="lock-closed" size="base" disabled>
                    {{ __('Board Closed') }}
                </flux:button>
            @elseif($this->isReadOnlyObserver)
                <flux:button icon="eye" size="base" disabled>
                    {{ __('Read Only') }}
                </flux:button>
            @else
                <flux:button
                ...
                >
                    {{ $currentBoardDecision ? __('Edit Decision') : __('Make Decision') }}
                </flux:button>
            @endif
        </div>
    </div>
    ...
    @if($this->isReadOnlyObserver)
        <div class="mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
            {{ __('This zonal transfer board is being viewed from the provincial level in read-only mode. You can review the application, scores, and prior decisions, but you cannot make or edit decisions from here.') }}
        </div>
    @endif

    <div class="mb-8 rounded-3xl border border-slate-300 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div class="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
                <div class="rounded-2xl border border-slate-300 bg-slate-50 px-5 py-3 text-center dark:border-zinc-700 dark:bg-zinc-800/60">
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-zinc-400">{{ __('Total') }}</p>
        ...
        @if($showScoreDetails)
            <div class="mt-5 space-y-4 border-t border-slate-200 pt-5 dark:border-zinc-700">
                            ...
                            @if(!empty($scoreItem['segments']))
                                <div class="mt-3 space-y-2">
                                    @foreach($scoreItem['segments'] as $segment)
                                        <div class="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900">
                                    ...
                                    @foreach($scoreItem['achievements'] as $achievement)
                                        <div class="rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs dark:border-zinc-700 dark:bg-zinc-900">
                                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                ...
                                                @unless($this->isReadOnlyObserver)
                                                    <flux:button
                                                        type="button"
                                                ...
                                                @else
                                                    <flux:badge variant="neutral" size="xs" class="uppercase tracking-tighter">{{ __('Read Only') }}</flux:badge>
                                                @endunless
                                            </div>
    ...
    @if($currentBoardDecision)
        @php
            $boardDecision = $currentBoardDecision;
        ...
        <div class="xl:col-span-8 space-y-8">
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-300 dark:border-zinc-700 shadow-sm overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Full Name') }}</p>
                    ...
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Employee ID') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $application->employee_id }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Teacher Category') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $teacher?->teacherCategory?->name ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Teacher Type') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $teacher?->teacherType?->type_name ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Appointment Subject') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $teacher?->appointmentSubject?->name_en ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Current Teaching Subject') }}</p>
                    ...
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Secondary Subject') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $teacher?->secondarySubject?->name_en ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Submitted Date') }}</p>
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-300 dark:border-zinc-700 shadow-sm overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Transfer Policy') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $application->policy?->title ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Transfer Category') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $application->display_category_name }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Target Province') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $application->targetProvince?->name ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Reason') }}</p>
                        <p class="mt-1 text-base font-semibold text-slate-700 dark:text-zinc-200">{{ $application->reason?->title ?? __('N/A') }}</p>
                    </div>
                    <div class="md:col-span-2">
                        <div class="rounded-2xl border border-slate-300 dark:border-zinc-700 bg-slate-50/50 dark:bg-zinc-800/20 p-5">
                            <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="xl:col-span-4 space-y-8">
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-300 dark:border-zinc-700 shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Institution / Office') }}</p>
                        <p class="mt-1 text-base font-black text-slate-900 dark:text-white">{{ $application->currentWorkplace?->office_name ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Authority') }}</p>
                    ...
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Zonal Office') }}</p>
                    ...
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Divisional Office') }}</p>
                    ...
                    <div class="pt-4 border-t border-slate-300 dark:border-zinc-700">
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Working Period At Station') }}</p>
            ...
            <div class="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-300 dark:border-zinc-700 shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Service') }}</p>
                    ...
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Rank') }}</p>
                        <p class="mt-1 text-sm font-semibold text-slate-700 dark:text-zinc-200">{{ $application->appointment?->rank?->rank_name ?? __('N/A') }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Position') }}</p>
                    ...
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('Gender') }}</p>
    ...
    <div class="mt-8 bg-white dark:bg-zinc-900 rounded-3xl border border-slate-300 dark:border-zinc-700 shadow-sm overflow-hidden">
        ...
        @if($showPreferenceDistanceMap && $hasPreferenceDistanceMap)
            <div class="border-b border-slate-200 bg-slate-50/40 p-6 dark:border-zinc-700 dark:bg-zinc-800/20">
        ...
        <div class="p-6 grid grid-cols-1 lg:grid-cols-2 gap-4">
            @forelse($application->preferences as $preference)
                ...
                @endphp
                <div class="rounded-2xl border border-slate-300 dark:border-zinc-700 bg-slate-50/60 dark:bg-zinc-800/20 p-5 hover:bg-slate-50 dark:hover:bg-zinc-800/35 transition-all duration-200">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex items-start gap-4">
                            <div class="w-11 h-11 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 flex items-center justify-center text-sm font-black text-slate-700 dark:text-zinc-200">
                    ...
                    @if($destinationInstitution?->id)
                        <div class="mt-4 pt-4 border-t border-slate-300 dark:border-zinc-700 flex items-center justify-between gap-3">
                            <div class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-zinc-400">{{ __('School transfer insight') }}</div>
                ...
                <div class="rounded-2xl border border-slate-300 dark:border-zinc-700 bg-slate-50/60 dark:bg-zinc-800/20 p-5 hover:bg-slate-50 dark:hover:bg-zinc-800/35 transition-all duration-200">
                    <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        ...
                        <div class="grid grid-cols-3 gap-2 md:min-w-[12rem]">
                            <div class="rounded-xl border border-slate-300 bg-white p-3 text-center dark:border-zinc-700 dark:bg-zinc-900">
                    ...
                    <div class="mt-4 pt-4 border-t border-slate-300 dark:border-zinc-700 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div class="mt-8 bg-white dark:bg-zinc-900 rounded-3xl border border-slate-300 dark:border-zinc-700 shadow-sm overflow-hidden">
        ...
        <div class="p-6 space-y-4">
            @foreach($application->policy?->steps ?? collect() as $step)
                @php
                    $timelineDecision = $this->timelineDecisionForStep($step);
                    $stepPending = in_array($application->status, ['submitted', 'processing']) && $application->current_step === $step->step_order && !$timelineDecision;
                @endphp
                <div class="rounded-2xl border border-slate-300 dark:border-zinc-700 p-4 {{ $stepPending ? 'bg-blue-50/60 dark:bg-blue-900/15' : 'bg-slate-50/50 dark:bg-zinc-800/20' }}">
                    <div class="flex flex-col lg:flex-row lg:items-start justify-between gap-3">
                        <div class="flex items-start gap-3">
                            <div class="w-8 h-8 rounded-full bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 flex items-center justify-center text-xs font-black text-slate-700 dark:text-zinc-200">
                                {{ $step->step_order }}
                            </div>
                            <div>
                                <p class="text-sm font-black text-slate-900 dark:text-white">{{ $step->officeLevel?->office_level_name }} {{ __('Review') }}</p>
                                @if($timelineDecision)
                                    <p class="mt-2 text-sm text-slate-600 dark:text-zinc-300">{{ $timelineDecision['remarks'] }}</p>
                                    <p class="mt-2 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-zinc-400">
                                        {{ $timelineDecision['source'] }} · {{ $timelineDecision['official'] }}
                                        @if($timelineDecision['date'])
                                            <span class="mx-1 text-slate-300 dark:text-zinc-600">|</span>
                                            {{ $timelineDecision['date']->format('M d, Y') }}
                                        @endif
                                    </p>
                                @elseif($stepPending)
                        ...
                        <div class="flex items-center gap-2">
                            @if($timelineDecision)
                                <flux:badge :color="$timelineDecision['color']" size="sm" class="uppercase tracking-tighter font-black">{{ $timelineDecision['label'] }}</flux:badge>
                            @elseif($stepPending)
    ...
    <flux:modal wire:model="showDecisionModal" name="decision-modal" class="w-full max-w-4xl rounded-4xl border border-slate-300/70 dark:border-zinc-700">
        <div class="space-y-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="max-w-2xl">
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-[16rem,1fr]">
                <div class="rounded-[1.75rem] border border-slate-300 bg-slate-50/70 p-5 dark:border-zinc-700 dark:bg-zinc-900/70">
                    <div>
                        <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-zinc-400">{{ __('Decision Path') }}</p>
                    ...
                    <div class="mt-5 space-y-3">
                        <button
                            ...
                            class="w-full rounded-[1.25rem] border px-4 py-4 text-left transition-all duration-200 {{ $decisionOutcome === 'approved' ? 'border-emerald-500 bg-emerald-50 shadow-sm shadow-emerald-500/10 dark:border-emerald-500/70 dark:bg-emerald-500/10' : 'border-slate-300 bg-white hover:border-slate-300 dark:border-zinc-700 dark:bg-zinc-950 dark:hover:border-zinc-700' }}"
                        >
                            <div class="flex items-start justify-between gap-3">
                                <div>
                ...
                <div class="rounded-[1.75rem] border border-slate-300 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <div class="flex justify-end border-t border-slate-300 pt-5 dark:border-zinc-700">
