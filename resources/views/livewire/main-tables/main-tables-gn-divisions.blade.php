    ...
    <x-main-tables.layout>
        <div class="max-w-[1400px] mx-auto pb-12 px-4 lg:px-0">
                        ...
                        <div class="flex justify-between items-start mb-4">
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                <span class="text-[10px] font-bold uppercase tracking-tighter {{ $data->active_status ? 'text-emerald-600' : 'text-rose-600' }}">
                                    {{ $data->active_status ? 'Active' : 'Offline' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="absolute inset-x-0 -bottom-2 flex justify-center opacity-0 group-hover:opacity-100 group-hover:bottom-4 transition-all duration-300 pointer-events-none group-hover:pointer-events-auto">
                            <div class="flex items-center gap-1 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-slate-200 dark:border-slate-700 p-1.5 rounded-2xl shadow-xl">
                                ...
                                <flux:button 
                                    ...
                                    wire:confirm="Toggle status for {{ $data->gn_division_name }}?"
                                    size="sm" 
                                    variant="ghost" 
            ...
            <div class="mt-12 px-4">
                {{ $gnDivisionList->links() }}
            </div>
        </div>

        {{-- MODAL: ADD NEW --}}
        <flux:modal wire:model="showModelNewGNDivision" name="add-new-gn-division" class="w-full max-w-lg rounded-[2.5rem] p-10">
            <div class="space-y-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="lg" class="!font-black uppercase tracking-tight leading-none">{{ __('New GN Division') }}</flux:heading>
                        <flux:text size="sm">{{ __('Define a new geographic administrative unit.') }}</flux:text>
                    </div>
                </div>
                ...
                <form wire:submit.prevent="addNewGNDivision" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-2 gap-4">
                        <flux:field>
                            <flux:input label="Division ID" wire:model.live="gnDivisionId" mask="GND99999" placeholder="GND00001" class="!rounded-xl" />
                        </flux:field>
                        <flux:field>
                            <flux:input label="Division Code" wire:model.live="gnDivisionCode" placeholder="Code" class="!rounded-xl" />
                        </flux:field>
                    </div>

                    <flux:field>
                        <flux:select label="DS Office" wire:model.live="dsOfficeId" class="!rounded-xl">
                            <option value="">{{ __('Select DS Office') }}</option>
                            @foreach ($dsOfficeOption as $dsOffice)
                                <option value="{{ $dsOffice->dso_id }}">{{ $dsOffice->dso_name }}</option>
                            @endforeach
                        </flux:select>
                    </flux:field>

                    <flux:field>
                        <flux:input label="GN Division Name" wire:model.live="gnDivisionName" placeholder="Enter Full Name" class="!rounded-xl" />
                    </flux:field>
                    ...
                    <div class="flex gap-3 pt-6">
                        ...
                        <flux:button type="submit" variant="primary" class="flex-[2] !rounded-xl">{{ __('Register Division') }}</flux:button>
                    </div>
                </form>
            </div>
        </flux:modal>
        ...
        {{-- MODAL: EDIT --}}
        <flux:modal wire:model="showModelEditGNDivision" name="edit-gn-division" class="w-full max-w-lg rounded-[2.5rem] p-10">
            <div class="space-y-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="lg" class="!font-black uppercase tracking-tight leading-none">{{ __('Edit GN Division') }}</flux:heading>
                        <flux:text size="sm">{{ __('Modify existing geographic unit data.') }}</flux:text>
                    </div>
                </div>
                ...
                <form wire:submit.prevent="updateGNDivision" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-2 gap-4">
                        <flux:field>
                            <flux:input disabled label="Division ID" wire:model.live="updateGNDivisionId" mask="GND99999" class="!rounded-xl bg-slate-50 opacity-60" />
                        </flux:field>
                        <flux:field>
                            <flux:input label="Division Code" wire:model.live="updateGNDivisionCode" class="!rounded-xl" />
                        </flux:field>
                    </div>

                    <flux:field>
                        <flux:select label="DS Office" wire:model.live="updateDsOfficeId" class="!rounded-xl">
                            <option value="">{{ __('Select DS Office') }}</option>
                            @foreach ($dsOfficeOption as $dsOffice)
                                <option value="{{ $dsOffice->dso_id }}">{{ $dsOffice->dso_name }}</option>
                            @endforeach
                    ...
                    <flux:field>
                        <flux:input label="GN Division Name" wire:model.live="updateGNDivisionName" class="!rounded-xl" />
                    </flux:field>
