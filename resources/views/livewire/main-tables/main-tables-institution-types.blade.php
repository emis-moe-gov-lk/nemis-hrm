    ...
    <x-main-tables.layout>
        <div class="max-w-[1400px] mx-auto pb-12 px-4 lg:px-0">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                <div class="space-y-1">
                    <flux:heading size="xl" level="1" class="!font-black tracking-tight text-indigo-600 dark:text-indigo-400 uppercase">
                        {{ __('Institution Types') }}
                    </flux:heading>
                    <flux:subheading size="lg" class="flex items-center gap-2">
                        <flux:icon.building-office-2 variant="micro" class="text-slate-400" />
                        {{ __('Manage categories and classifications for system institutions') }}
                    </flux:subheading>
                </div>

                <div class="flex items-center gap-3">
                    <flux:modal.trigger name="add-new-institution-type">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                @forelse ($institution_types as $key => $data)
                    <div class="group relative flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-6 hover:shadow-2xl hover:shadow-indigo-500/10 hover:border-indigo-500/30 transition-all duration-500">
                        ...
                        <div class="mb-6 flex flex-col items-center text-center">
                            <div class="mb-4 w-16 h-16 rounded-3xl bg-indigo-50 dark:bg-indigo-900/30 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                                <flux:icon.building-library class="text-indigo-600 dark:text-indigo-400" />
                            </div>
                            ...
                            <p class="mt-3 text-xs text-slate-500 dark:text-slate-400 line-clamp-2 px-2">
                        ...
                        <div class="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                             <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs font-bold text-slate-400">#{{ $institution_types->firstItem() + $key }}</span>
                             </div>
                             
                             <div class="flex items-center gap-1">
                                <flux:modal.trigger wire:click="editInstitutionType({{ $data->id }})" name="edit-institution-type">
                                ...
                                <flux:button 
                                    ...
                                    wire:confirm="Update status for {{ $data->institution_types_name }}?"
                                    size="sm" 
                                    variant="ghost" 
                ...
                @empty
                    <div class="col-span-full py-24 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                        <flux:icon.building-office-2 size="xl" class="text-slate-300 mb-4" />
                        <p class="text-slate-400 font-black uppercase tracking-widest text-sm italic">{{ __('No Institution Types Found') }}</p>
                    </div>
                @endforelse
            </div>
        ...
        {{-- MODAL: ADD NEW --}}
        <flux:modal wire:model="showModelNewInstitutionType" name="add-new-institution-type" class="w-full max-w-lg rounded-[2.5rem] p-10">
            <div class="space-y-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="lg" class="!font-black uppercase tracking-tight leading-none">{{ __('New Type') }}</flux:heading>
                        <flux:text size="sm">{{ __('Define a new institution category.') }}</flux:text>
                    </div>
                </div>

                <form wire:submit.prevent="addNewInstitutionType" class="space-y-4 pt-2">
                    @csrf
                    <flux:field>
                        <flux:input label="Type ID" wire:model.live="institution_types_id" mask="ITID99" placeholder="ITID01" class="!rounded-xl" />
                    </flux:field>

                    <flux:field>
                        <flux:input label="Type Name" wire:model.live="institution_types_name" placeholder="e.g. University" class="!rounded-xl" />
                    </flux:field>

                    <flux:field>
                        <flux:textarea label="Description" wire:model.live="description" class="!rounded-xl" />
                    </flux:field>
                    ...
                    <div class="flex gap-3 pt-6">
                        ...
                        <flux:button type="submit" variant="primary" class="flex-[2] !rounded-xl shadow-lg shadow-indigo-500/20">{{ __('Create Type') }}</flux:button>
                    </div>
                </form>
            </div>
        </flux:modal>
        ...
        {{-- MODAL: EDIT --}}
        <flux:modal wire:model="showModelEditInstitutionType" name="edit-institution-type" class="w-full max-w-lg rounded-[2.5rem] p-10">
            <div class="space-y-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="lg" class="!font-black uppercase tracking-tight leading-none">{{ __('Edit Type') }}</flux:heading>
                        <flux:text size="sm">{{ __('Modify existing institution type details.') }}</flux:text>
                    </div>
                </div>

                <form wire:submit.prevent="updateInstitutionType" class="space-y-4 pt-2">
                    @csrf
                    <flux:field>
                        <flux:input disabled label="Type ID" wire:model.live="update_institution_types_id" mask="ITID99" class="!rounded-xl bg-slate-50 opacity-60" />
                    </flux:field>

                    <flux:field>
                        <flux:input label="Type Name" wire:model.live="update_institution_types_name" class="!rounded-xl" />
                    </flux:field>

                    <flux:field>
                        <flux:textarea label="Description" wire:model.live="update_description" class="!rounded-xl" />
                    </flux:field>
