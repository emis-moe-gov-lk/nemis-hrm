            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-8 px-4">
                    ...
                    <div class="group relative flex flex-col bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-[3rem] p-8 hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_40px_80px_-15px_rgba(0,0,0,0.5)] hover:border-indigo-500/50 transition-all duration-500">
                        ...
                        <div class="flex justify-between items-start mb-6">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[10px] font-black uppercase tracking-widest px-4 py-1 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 rounded-full border border-indigo-200 dark:border-indigo-800">
                                    Order: {{ $data->position_order }}
                                </span>
                            </div>
                        ...
                        {{-- Service Parent Info --}}
                        <div class="mb-6 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50">
                            <div class="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">{{ __('Linked Service') }}</div>
                            <div class="flex items-center gap-2">
                                <flux:icon.briefcase variant="micro" class="text-indigo-500" />
                                <span class="text-xs font-bold text-slate-700 dark:text-slate-300">
                                    {{ $data->service->service_name }}
                                    <span class="text-[10px] text-slate-500 font-medium">({{ $data->service_id }})</span>
                                </span>
                            </div>
                        </div>
                        ...
                        {{-- Description --}}
                        <div class="mb-8">
                            <div class="flex items-start gap-2 text-slate-500 dark:text-slate-500">
                ...
                @empty
                    <div class="col-span-full py-32 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/40 rounded-[4rem] border-2 border-dashed border-slate-300 dark:border-slate-700">
                        ...
                        <p class="text-slate-500 font-black uppercase tracking-[0.3em] text-sm">{{ __('No Positions Defined') }}</p>
                    </div>
                @endforelse
            </div>
        ...
        <flux:modal wire:model="showModelNewPosition" name="add-new-position" class="w-full max-w-lg rounded-[3rem] p-12">
            <div class="space-y-8">
                ...
                <form wire:submit.prevent="addNewPosition" class="space-y-5">
                    ...
                    <flux:field>
                        <flux:input type="number" label="Position Order" wire:model.live="positionOrder" placeholder="e.g. 1" class="rounded-[1.25rem]! py-3!" />
                    </flux:field>

                    <flux:field>
                        <flux:textarea label="Role Description" wire:model.live="positionDescription" placeholder="Outline the primary duties of this rank..." class="rounded-[1.25rem]!" rows="3" />
                    </flux:field>
        ...
        <flux:modal wire:model="showModelEditPosition" name="edit-position" class="w-full max-w-lg rounded-[3rem] p-12">
            <div class="space-y-8">
                ...
                <form wire:submit.prevent="updatePosition" class="space-y-5">
                    ...
                    <flux:field>
                        <flux:input type="number" label="Position Order" wire:model.live="updatePositionOrder" class="rounded-[1.25rem]! py-3!" />
                    </flux:field>
