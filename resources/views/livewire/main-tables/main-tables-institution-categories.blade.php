    ...
    <x-main-tables.layout>
        <div class="max-w-[1400px] mx-auto pb-12 px-4 lg:px-0">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                <div class="space-y-1">
                    <flux:heading size="xl" level="1" class="!font-black tracking-tight text-emerald-600 dark:text-emerald-400 uppercase">
                ...
                <div class="flex items-center gap-3">
                    <flux:modal.trigger name="add-new-ins-category">
                        <flux:button icon="plus" color="primary" class="w-full sm:w-auto !rounded-2xl shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all">
                            {{ __('Add Category') }}
                        </flux:button>
                    </flux:modal.trigger>
                </div>
                        ...
                        <div class="absolute inset-x-0 -bottom-2 flex justify-center opacity-0 group-hover:opacity-100 group-hover:bottom-4 transition-all duration-300 pointer-events-none group-hover:pointer-events-auto">
                            <div class="flex items-center gap-1 bg-white/95 dark:bg-slate-800/95 backdrop-blur-md border border-slate-200 dark:border-slate-700 p-1.5 rounded-2xl shadow-xl">
                                ...
                                <flux:button 
                                    ...
                                    wire:confirm="Change status for {{ $data->institution_category_name }}?"
                                    size="sm" 
                                    variant="ghost" 
        ...
        {{-- MODAL: ADD NEW --}}
        <flux:modal wire:model="showModelNewInsCategory" name="add-new-ins-category" class="w-full max-w-lg rounded-[2.5rem] p-10">
            <div class="space-y-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="lg" class="!font-black uppercase tracking-tight leading-none">{{ __('New Category') }}</flux:heading>
                        <flux:text size="sm">{{ __('Create a new classification for institutions.') }}</flux:text>
                    </div>
                </div>

                <form wire:submit.prevent="addNewInsCategory" class="space-y-4 pt-2">
                    @csrf
                    <flux:field>
                        <flux:input label="Category ID" wire:model.live="institution_category_id" mask="ICID999" placeholder="ICID001" class="!rounded-xl" />
                    </flux:field>

                    <flux:field>
                        <flux:input label="Category Name" wire:model.live="institution_category_name" placeholder="Enter Category Name" class="!rounded-xl" />
                    </flux:field>

                    <flux:field>
                        <flux:textarea label="Description" wire:model.live="description" placeholder="Briefly describe this category..." class="!rounded-xl" rows="4" />
                    </flux:field>
                    ...
                    <div class="flex gap-3 pt-6">
                        ...
                        <flux:button type="submit" variant="primary" class="flex-[2] !rounded-xl shadow-lg shadow-emerald-500/20">{{ __('Add Category') }}</flux:button>
                    </div>
                </form>
            </div>
        </flux:modal>
        ...
        {{-- MODAL: EDIT --}}
        <flux:modal wire:model="showModelEditInsCategory" name="edit-ins-category" class="w-full max-w-lg rounded-[2.5rem] p-10">
            <div class="space-y-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="lg" class="!font-black uppercase tracking-tight leading-none">{{ __('Edit Category') }}</flux:heading>
                        <flux:text size="sm">{{ __('Modify existing classification settings.') }}</flux:text>
                    </div>
                </div>

                <form wire:submit.prevent="updateInsCategory" class="space-y-4 pt-2">
                    @csrf
                    <flux:field>
                        <flux:input disabled label="Category ID" wire:model.live="update_institution_category_id" mask="ICID999" class="!rounded-xl bg-slate-50 opacity-60" />
                    </flux:field>

                    <flux:field>
                        <flux:input label="Category Name" wire:model.live="update_institution_category_name" class="!rounded-xl" />
                    </flux:field>
                    ...
                    <div class="flex gap-3 pt-6">
                        ...
                        <flux:button type="submit" variant="primary" class="flex-[2] !rounded-xl shadow-lg shadow-emerald-500/20">{{ __('Save Changes') }}</flux:button>
                    </div>
                </form>
            </div>
        </flux:modal>
