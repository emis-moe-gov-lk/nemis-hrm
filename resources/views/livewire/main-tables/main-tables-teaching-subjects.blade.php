<section class="w-full">
    <x-main-tables.layout>
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                @forelse ($teaching_subjects as $key => $data)
                    <div class="group relative bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-4xl p-6 shadow-sm hover:shadow-xl hover:border-indigo-200 dark:hover:border-indigo-900 transition-all duration-300">
                        
                        {{-- Card Header: ID & Status --}}
                        <div class="flex justify-between items-start mb-4">
                            <div class="flex flex-col">
                                <span class="text-[10px] font-black uppercase tracking-widest text-slate-500">{{ $data->subject_id }}</span>
                        ...
                        <div class="space-y-3 mb-6">
                            <div>
                                ...
                                <p class="text-sm font-medium text-slate-500 dark:text-slate-500 mt-1">{{ $data->name_si }}</p>
                                <p class="text-sm font-medium text-slate-500 dark:text-slate-500">{{ $data->name_ta }}</p>
                            </div>

                            <div class="flex items-center gap-2">
                                <span class="px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[10px] font-bold text-slate-600 dark:text-slate-500 uppercase">
                                    Type: {{ $data->type_name }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-2 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl mb-6">
                            <div class="flex justify-between items-center">
                                <span class="text-[9px] font-black text-slate-500 uppercase">Grade Mask</span>
                                <code class="text-[10px] text-indigo-600 dark:text-indigo-400 font-mono tracking-tighter">{{ $data->grade_mask }}</code>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-[9px] font-black text-slate-500 uppercase">Lang Mask</span>
                ...
                @empty
                    <div class="col-span-full py-20 bg-slate-50 dark:bg-slate-800/50 rounded-[3rem] border-2 border-dashed border-slate-300 dark:border-slate-700 flex flex-col items-center">
        ...
        {{-- MODAL: ADD NEW --}}
        <flux:modal wire:model="showModelNewTeachingSubject" name="add-new-teaching-subject" class="md:w-[32rem] rounded-4xl">
            <div class="space-y-6">
                <div>
                ...
                <form wire:submit.prevent="addNewTeachingSubject" class="space-y-4">
                    ...
                    <div class="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl space-y-4">
                        <flux:heading size="sm" class="font-black uppercase text-slate-500 text-[10px]">Config Masks</flux:heading>
        ...
        {{-- MODAL: EDIT --}}
        <flux:modal wire:model="showModelEditTeachingSubject" name="edit-teaching-subject" class="md:w-[32rem] rounded-4xl">
            <div class="space-y-6">
                <div>
