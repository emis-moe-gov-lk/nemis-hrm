<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    <x-page-header
        title="Institution Groups - Teachers"
        subtitle="Teachers in groups assigned to your user account."
        ...
        @if (!$hasAssignedGroups)
            <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center dark:border-slate-700 dark:bg-slate-900/50">
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-500">
                    {{ __('Teacher list is available only when you are assigned to at least one institution group.') }}
                </p>
            </div>
        @else
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                        <thead class="bg-slate-50 dark:bg-slate-800/60">
                            <tr>
                                <th class="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">{{ __('Teacher Name') }}</th>
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                            @forelse ($teachers as $teacher)
                                <tr>
                                    <td class="px-4 py-3 text-slate-700 dark:text-slate-200">{{ $teacher->employee?->name_with_initials ?? 'N/A' }}</td>
                                    <td class="px-4 py-3 text-slate-500 dark:text-slate-500">{{ $teacher->position?->position_name ?? 'Teacher' }}</td>
                                    <td class="px-4 py-3 text-slate-700 dark:text-slate-200">
                                        {{ $teacher->institution?->name ?? 'N/A' }}
                                        <p class="text-xs text-slate-500 dark:text-slate-500">
                                            {{ __('Census No') }}: {{ $teacher->institution?->census_no ? str_pad($teacher->institution->census_no, 5, '0', STR_PAD_LEFT) : 'N/A' }}
                                        </p>
                                    </td>
                                    <td class="px-4 py-3 text-slate-700 dark:text-slate-200">
                                        {{ implode(', ', $institutionGroupMap[$teacher->workplace_id] ?? []) ?: 'N/A' }}
                                    </td>
                                    <td class="px-4 py-3 text-slate-500 dark:text-slate-500">
                                        {{ optional($teacher->appoint_date)->format('Y-m-d') ?? 'N/A' }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-10 text-center text-slate-500 dark:text-slate-500">
                                        {{ __('No teachers found for your assigned group(s).') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
