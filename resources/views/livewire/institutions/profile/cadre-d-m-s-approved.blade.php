<section class="w-full" x-data="{ selectedRow: null }">
    ...
    <header class="space-y-6 mb-8">
        <div>
            <flux:heading size="xl" level="1" class="!text-2xl font-black tracking-tight text-slate-800 uppercase">
                {{ __('Staff Summary') }}
            </flux:heading>
    ...
    <x-institutions.institution-layout :institutionId="$id">
        ...
        <div class="flex justify-end w-full">
            @if ($circular)
            <div class="flex items-center gap-4 px-4 py-2 bg-white border border-slate-200 rounded-xl shadow-sm w-fit">
                <div class="text-right">
                    <span class="text-[10px] font-black text-slate-400 uppercase block leading-none mb-1">{{ __('Circular') }}</span>
                ...
                <div class="text-right">
                    <span class="text-[10px] font-black text-slate-400 uppercase block leading-none mb-1">{{ __('Date') }}</span>
        ...
        <div class="md:hidden space-y-10 mt-6">
            @forelse($groupedRows as $typeId => $items)
                @php $currentType = $typeLabels[$typeId] ?? ['label' => 'General', 'class' => 'bg-slate-50 text-slate-700 border-slate-100']; @endphp
                
                <div class="space-y-4">
                    <div class="px-2 py-1 inline-block rounded-md border text-[10px] font-black uppercase tracking-[0.1em] {{ $currentType['class'] }}">
                        {{ $currentType['label'] }}
                    </div>
                    ...
                    @foreach($items as $row)
                        ...
                        <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm transition-all"
                            ...
                            <div class="p-4 flex justify-between items-start cursor-pointer" @click="selectedRow = (selectedRow === '{{ $uniqueId }}' ? null : '{{ $uniqueId }}')">
                                <div class="flex gap-3">
                                     ...
                                     <div>
                                        ...
                                        <div class="text-[10px] text-slate-400 font-medium uppercase mt-1">{{ $row['medium_name'] }}</div>
                                    </div>
                                </div>
                            ...
                            {{-- Stats Strip --}}
                            <div class="px-4 py-3 bg-slate-50/50 border-y border-slate-100 grid grid-cols-3 gap-2 text-center">
                                <div class="border-r border-slate-200">
                                    <dt class="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{{ __('Approved') }}</dt>
                                    <dd class="text-xs font-bold text-slate-700">{{ $row['approved_posts'] }}</dd>
                                </div>
                                <div class="border-r border-slate-200">
                                    <dt class="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{{ __('Filled') }}</dt>
                                    <dd class="text-xs font-bold text-slate-700">{{ $row['filled_posts'] }}</dd>
                                </div>
                                <div>
                                    <dt class="text-[9px] font-black text-slate-400 uppercase tracking-tighter">{{ __('Gap') }}</dt>
                            ...
                            <div x-show="selectedRow === '{{ $uniqueId }}'" x-collapse x-cloak>
                                <div class="p-4 bg-white">
                                    <div class="text-[9px] font-black text-slate-400 uppercase mb-3 tracking-widest">{{ __('Staff breakdown by seniority') }}</div>
                                    ...
                                    <div class="space-y-2">
                                        @foreach($row['teachers'] as $tIndex => $teacher)
                                            ...
                                            @php
                                                ...
                                                $routeName = match($teacher->currentAppointment->service_id) {
                                            ...
                                            <a href="{{ route($routeName, $teacher->employee->id) }}" class="mx-1">
                                                <div class="p-3 rounded-xl border {{ $isExcess ? 'bg-red-50 border-red-200' : 'bg-slate-50/50 border-slate-200' }}">
                                                    <div class="flex justify-between items-start gap-2">
                                                        <div class="text-[11px] font-bold {{ $isExcess ? 'text-red-700' : 'text-slate-800' }}">
                                                            {{ $teacher->employee->name_with_initials }}
                                                        </div>
                                                        @if($isExcess)
                                                            <span class="flex-shrink-0 text-[8px] bg-red-600 text-white px-1.5 py-0.5 rounded font-black uppercase">Excess</span>
                                                        @endif
                                                    </div>
                                                    <div class="flex justify-between mt-2 text-[10px]">
                                                        <span class="font-mono text-slate-400">{{ $teacher->employee->nic }}</span>
            ...
            @empty
                <div class="py-12 text-center text-slate-400">{{ __('No staff records found') }}</div>
            @endforelse

            {{-- Mobile Grand Total Footer --}}
            <div class="p-5 bg-slate-900 rounded-3xl text-white shadow-xl mt-8">
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach($groupedRows as $typeId => $items)
                        ...
                        <tr class="{{ $currentType['class'] }} border-y border-slate-100">
                        ...
                        @foreach($items as $row)
                            ...
                            <tr class="hover:bg-blue-50/30 transition-colors cursor-pointer" @click="selectedRow = (selectedRow === '{{ $uniqueId }}' ? null : '{{ $uniqueId }}')" :class="selectedRow === '{{ $uniqueId }}' ? 'bg-blue-50/50' : ''">
                                ...
                                <td class="px-5 py-4">
                                    <div class="text-sm font-bold text-slate-900 leading-tight">{{ $row['subject_name'] }}</div>
                                    <div class="text-[10px] text-slate-400 font-medium uppercase mt-0.5 tracking-tight">{{ $row['medium_name'] }}</div>
                                </td>
                                    ...
                                    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                                        @if($row['teachers']->isNotEmpty())
                                            ...
                                            <table class="w-full text-xs">
                                                <thead class="bg-slate-50 border-b border-slate-200">
                                                    <tr>
                                                        <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase">#</th>
                                                        <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase">Name</th>
                                                        <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase">NIC</th>
                                                        <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase text-right">Appointment Date</th>
                                                        <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase text-right">Service in School</th>
                                                        <th class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase text-right">action</th>
                                                    </tr>
                                                </thead>
                                                <tbody class="divide-y divide-slate-100">
                                                    @foreach($row['teachers'] as $tIndex => $teacher)
                                                        ...
                                                        <tr class="transition-colors {{ $isExcessRow ? 'bg-red-50/80' : 'hover:bg-slate-50/50' }}">
                                                            <td class="px-4 py-2.5 {{ $isExcessRow ? 'text-red-400' : 'text-slate-400' }}">
                                                                {{ $tIndex + 1 }}
                                                            </td>
                                        ...
                                        @else
                                            <div class="p-6 text-center text-slate-400 italic text-xs">{{ __('No teachers currently assigned.') }}</div>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                ...
                <tfoot class="bg-slate-900 text-white border-t-2 border-slate-800">
                    <tr>
                        <td colspan="2" class="px-5 py-6 text-xs font-black uppercase tracking-[0.2em] text-slate-400">{{ __('Grand Total Summary') }}</td>
                        ...
                        <td class="px-5 py-6 text-center"><span class="bg-slate-800 text-[9px] px-2 py-1 rounded font-black text-slate-400 uppercase">ALL SERVICES</span></td>
                    </tr>
                </tfoot>
            </table>
        </div>
