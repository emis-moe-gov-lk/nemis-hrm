<div class="max-w-6xl mx-auto py-8 px-4">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <flux:heading size="xl" level="1" class="text-2xl! font-black tracking-tight text-zinc-800 dark:text-zinc-100 uppercase">{{ __('Add DMS Approved Cadre') }}</flux:heading>
        </div>

        <div class="flex flex-col sm:flex-row gap-3">
            <div class="flex items-center gap-4 px-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm transition-all hover:border-blue-300 dark:hover:border-blue-700/50">
                <div class="text-center">
                    <span class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase block tracking-widest">{{ __('Workplace') }}</span>
                ...
                <div class="text-center">
                    <span class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase block tracking-widest">{{ __('Census') }}</span>
                    <span class="text-sm font-bold text-zinc-700 dark:text-zinc-300">{{ $institution->census_no }}</span>
                </div>
            </div>

            <div class="flex items-center gap-4 px-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm transition-all hover:border-emerald-300 dark:hover:border-emerald-700/50">
                <div class="text-center">
                    <span class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase block tracking-widest">{{ __('Circular') }}</span>
                    <span class="text-sm font-bold text-emerald-600 dark:text-emerald-400">{{ $circular->circular_no }}</span>
                </div>
                <flux:separator vertical class="h-8 dark:bg-zinc-800" />
                <div class="text-center">
                    <span class="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase block tracking-widest">{{ __('Date') }}</span>
    ...
    <form wire:submit.prevent="save" class="space-y-6">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[700px]">
                    <thead class="bg-zinc-50/80 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
                        <tr>
                            <th class="sticky left-0 z-10 bg-zinc-50/80 dark:bg-zinc-800 px-6 py-4 text-[11px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-400 border-r border-zinc-200 dark:border-zinc-800 shadow-[1px_0_0_0_var(--zinc-200)] dark:shadow-[1px_0_0_0_var(--zinc-800)]">
                                {{ __('Subject Details') }}
                            </th>
                            @foreach ($mediums as $medium)
                            <th class="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-400 text-center border-r border-zinc-100 dark:border-zinc-800 last:border-r-0">
                                {{ $medium->name }}
                            </th>
                            @endforeach
                            <th class="px-6 py-4 text-[11px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-400 text-center bg-zinc-100/30 dark:bg-zinc-800/30">
                                {{ __('Total') }}
                            </th>
                        </tr>
                    </thead>
                    ...
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                        ...
                        <tr class="{{ $typeLabels[$type]['class'] ?? 'bg-zinc-50 dark:bg-zinc-800/30' }}">
                            <td colspan="{{ count($mediums) + 2 }}" class="px-6 py-2.5 text-[10px] font-black uppercase tracking-[0.15em] border-y border-zinc-200/50 dark:border-zinc-800/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]">
                        ...
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors group">
                            {{-- Subject Name (Sticky) --}}
                            <td class="sticky left-0 z-10 bg-white dark:bg-zinc-900 group-hover:bg-zinc-50 dark:group-hover:bg-zinc-800/50 px-6 py-4 border-r border-zinc-100 dark:border-zinc-800 shadow-[1px_0_0_0_var(--zinc-100)] dark:shadow-[1px_0_0_0_var(--zinc-800)] transition-colors">
                                ...
                                <div class="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono mt-1 uppercase tracking-tighter bg-zinc-100 dark:bg-zinc-800 inline-block px-1.5 py-0.5 rounded border border-zinc-200 dark:border-zinc-700">ID: {{ $subject->subject_id }}</div>
                            </td>
                            ...
                            @endphp
                            <td class="px-3 py-2 border-r border-zinc-50 dark:border-zinc-800/50 last:border-r-0">
                                <flux:input
                                    type="number"
                                    min="0"
                                    size="sm"
                    ...
                    <tfoot class="bg-zinc-900 dark:bg-black text-white border-t-4 border-white dark:border-zinc-800 relative z-20 box-border border-b border-x border-r border-l">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
            <div class="flex items-start sm:items-center gap-3 text-zinc-500 dark:text-zinc-400 bg-blue-50/50 dark:bg-blue-900/10 p-3 rounded-xl border border-blue-100/50 dark:border-blue-900/30">
