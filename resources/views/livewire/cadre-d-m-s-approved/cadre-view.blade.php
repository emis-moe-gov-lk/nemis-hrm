<div class="max-w-6xl mx-auto py-8 px-4">
    {{-- Header Section --}}
    <header class="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
        <div class="space-y-6 w-full">
            <flux:heading size="xl" level="1" class="text-2xl! font-black tracking-tight text-zinc-800 dark:text-zinc-100 uppercase">
                {{ __('DMS Approved Cadre') }}
            </flux:heading>

            <div class="flex flex-col sm:flex-row gap-3 md:max-w-4xl">
                {{-- Workplace Card --}}
                <div class="flex-1 flex items-center gap-4 p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm transition-all hover:border-blue-300 dark:hover:border-blue-700/50 hover:shadow-md group">
                    ...
                    <div class="overflow-hidden">
                        <span class="block text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500 leading-none mb-1.5">
                ...
                {{-- Circular Card --}}
                @if ($activeCircular)
                <div class="flex-1 flex items-center gap-4 p-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm transition-all hover:border-emerald-300 dark:hover:border-emerald-700/50 hover:shadow-md group">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-[11px] text-zinc-500 dark:text-zinc-400 mt-0.5">
                ...
                @else
                <div class="flex-1 flex items-center gap-4 p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-2xl shadow-sm">
        ...
        <div class="flex gap-2 w-full md:w-auto">
            ...
            <flux:button
                ...
                variant="primary"
                size="sm"
                class="flex-1 md:flex-none shadow-md shadow-blue-500/20 dark:shadow-blue-900/30">
    ...
    <div class="md:hidden space-y-8">
        ...
        <div class="space-y-3">
            <div class="flex items-center gap-2 px-1">
                ...
                <h3 class="text-[10px] font-black uppercase tracking-widest text-zinc-500 dark:text-zinc-400">
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
                <div class="p-4 border-b border-zinc-50 dark:border-zinc-800/80">
                ...
                <div class="p-4 grid grid-cols-2 gap-y-4 gap-x-2">
                    ...
                    <div>
                        <dt class="text-[9px] uppercase font-bold text-zinc-400 dark:text-zinc-500 tracking-tighter">{{ $medium->name }}</dt>
                        <dd class="text-sm font-bold text-zinc-700 dark:text-zinc-300">{{ $row['medium_totals'][$medium->medium_id] ?? 0 }}</dd>
                    </div>
                    @endforeach
                </div>
                <div class="px-4 py-3 bg-zinc-50 dark:bg-zinc-800/50 flex justify-between items-center border-t border-zinc-100 dark:border-zinc-800">
                    <span class="text-[10px] font-black text-zinc-500 dark:text-zinc-400 uppercase tracking-widest">{{ __('Total') }}</span>
        ...
        @empty
        <div class="text-center py-10 bg-white dark:bg-zinc-900/50 rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-700 text-zinc-400 dark:text-zinc-500 text-sm">
        ...
        <div class="bg-zinc-900 dark:bg-black rounded-3xl p-6 text-white shadow-2xl ring-4 ring-zinc-900/10 dark:ring-white/5 relative overflow-hidden">
            ...
            <div class="relative z-10 flex items-center gap-2 mb-6">
                ...
                <h4 class="text-[10px] font-black uppercase text-zinc-400 dark:text-zinc-500 tracking-[0.2em]">{{ __('Grand Summary') }}</h4>
            </div>

            <div class="relative z-10 grid grid-cols-2 gap-y-6 gap-x-4 mb-6">
                @foreach ($mediums as $medium)
                <div class="border-l-2 border-zinc-700 dark:border-zinc-800 pl-3">
                    <div class="text-[10px] uppercase text-zinc-400 dark:text-zinc-500 font-bold mb-1">{{ $medium->name }}</div>
                    <div class="text-xl font-bold tracking-tight">{{ $mediumSums[$medium->medium_id] }}</div>
                </div>
                @endforeach
            </div>

            <div class="relative z-10 pt-5 border-t border-zinc-800 dark:border-zinc-800/80 flex justify-between items-end">
                <div>
    ...
    {{-- 2. WEB VIEW --}}
    <div class="hidden md:block bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
        <table class="w-full text-left border-collapse">
            <thead class="bg-zinc-50/80 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
                <tr>
                    <th class="px-5 py-4 text-[11px] font-black uppercase text-zinc-500 dark:text-zinc-400 border-r border-zinc-200 dark:border-zinc-800 tracking-widest">
                        {{ __('Subject Details') }}
                    </th>
                    @foreach ($mediums as $medium)
                    <th class="px-3 py-4 text-[10px] font-black uppercase text-center border-r border-zinc-100 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 last:border-r-0">
                        {{ $medium->name }}
                    </th>
                    @endforeach
                    <th class="px-5 py-4 text-[11px] font-black uppercase text-center bg-zinc-100/30 dark:bg-zinc-800/30 text-zinc-500 dark:text-zinc-400 tracking-widest">
                        {{ __('Total') }}
                    </th>
                </tr>
            </thead>
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                ...
                <tr class="{{ $typeLabels[$type]['light'] ?? 'bg-zinc-50 dark:bg-zinc-800/30' }}">
                    <td colspan="{{ count($mediums) + 3 }}" class="px-5 py-2.5 border-y border-zinc-200/50 dark:border-zinc-800/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.1)]">
                ...
                <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors group">
                    <td class="px-5 py-4 border-r border-zinc-50 dark:border-zinc-800/50">
                        ...
                        <div class="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono mt-1 uppercase tracking-tighter bg-zinc-100 dark:bg-zinc-800 inline-block px-1.5 py-0.5 rounded border border-zinc-200 dark:border-zinc-700">ID: {{ $row['subject']->subject_id }}</div>
                    </td>
                    @foreach ($mediums as $medium)
                    <td class="px-3 py-4 text-center text-sm font-semibold text-zinc-600 dark:text-zinc-300 border-r border-zinc-50 dark:border-zinc-800/50 last:border-r-0">
                ...
                @empty
                <tr>
                    <td colspan="{{ count($mediums) + 2 }}" class="px-5 py-10 text-center text-zinc-400 dark:text-zinc-500 text-sm bg-white dark:bg-zinc-900">
                        {{ __('No data available') }}
                    </td>
                </tr>
                @endforelse
            ...
            @if ($grandTotal > 0)
            <tfoot class="bg-zinc-900 dark:bg-black text-white border-t-4 border-white dark:border-zinc-800 relative z-10 box-border border-b border-x border-r border-l">
