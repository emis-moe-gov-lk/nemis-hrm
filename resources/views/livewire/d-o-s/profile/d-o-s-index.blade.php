            ...
            <div class="antialiased min-h-screen">
                ...
                <div class="bg-white dark:bg-gray-800 p-2 rounded-b-lg">
                    ...
                    <div class="space-y-6">
                        ...
                        <livewire:employees.personal-cultural :peopleId="$dos->people_id" :canEdit="auth()->user()->can('dos.profile.personal-cultural.update')" />

                        {{-- 2. Health information --}}
                        <livewire:employees.health-information :peopleId="$dos->people_id" :canEdit="auth()->user()->can('dos.profile.health.update')" />

                        {{-- 3. Contact Information --}}
                        <livewire:employees.contact-information :peopleId="$dos->people_id" :canEdit="auth()->user()->can('dos.profile.contact-information.update')" />

                        {{-- 4. Location Information --}}
                        <livewire:employees.location-information :peopleId="$dos->people_id" :canEdit="auth()->user()->can('dos.profile.location-information.update')" />

                        {{-- 4. Temporary Location Information --}}
                        <livewire:employees.temporary-location-information :peopleId="$dos->people_id" :canEdit="auth()->user()->can('dos.profile.temporary-location-information.update')" />
                        ...
                        <section class="mt-8 pt-6 border-t border-dashed border-gray-200 dark:border-gray-800">
                            <div class="bg-gray-50 dark:bg-gray-900/40 rounded-xl p-4 flex items-center justify-between group">
                                ...
                                <flux:button 
                                    ...
                                    x-on:click="window.navigator.clipboard.writeText('{{ $dos->nic_hash }}'); Flux.toast({ variant: 'success', text: 'Hash copied to clipboard' })"
                                />
                            </div>
