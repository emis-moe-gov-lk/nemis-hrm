<div class="p-4 md:p-8 max-w-7xl mx-auto min-h-screen">
    {{-- Header Section --}}
    <x-page-header
        title="Development Officer Directory"
        subtitle="Manage Development Officer profiles and localized registry."
        icon="briefcase"
        ...
        <div class="w-full xl:w-auto">
            <flux:modal.trigger name="search-profile">
                <flux:button variant="filled" icon="magnifying-glass" class="w-full sm:w-auto h-10 bg-white! dark:bg-slate-900! text-slate-700! dark:text-slate-300! border border-slate-300 dark:border-slate-700 shadow-sm hover:border-indigo-300! transition-all group">
                    <span class="group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">Search DOS Officers...</span>
                </flux:button>
            </flux:modal.trigger>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="group relative bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl p-5 transition-all duration-300 hover:shadow-xl hover:border-indigo-200 flex flex-col lg:flex-row lg:items-center gap-6">
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto lg:ml-auto border-t lg:border-t-0 border-slate-200 dark:border-slate-700 pt-4 lg:pt-0">
                <flux:button href="{{ route('dos.profile.index', $employee->id) }}" size="sm" class="w-full lg:w-auto rounded-xl! bg-indigo-600! text-white! font-bold px-4">View</flux:button>
                <flux:dropdown>
        ...
        <div class="py-20 flex flex-col items-center justify-center bg-white dark:bg-slate-900 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-[3rem] transition-all">
            ...
            {{-- Text Content --}}
            <h3 class="text-xl font-bold text-slate-800 dark:text-slate-200">No Development Officers Found</h3>
        ...
        <div class="space-y-2 mt-4">
            ...
            <a href="{{ route('dos.profile.index', $dos->id) }}" class="flex items-center gap-4 p-4 rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-900/10 border border-transparent hover:border-indigo-100 transition-all">
                ...
                <div>
                    ...
                    <p class="text-xs text-indigo-400">{{ $dos->currentAppointment?->workplace?->office_name ?? 'N/A' }}</p>
                </div>
            </a>
            @empty
