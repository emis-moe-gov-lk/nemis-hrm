<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Create Sri Lanka Administrative Service Officer') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Create SLAS profile and account') }}</flux:subheading>
...
<form wire:submit.prevent="save" class="mt-6 max-w-2xl space-y-6">
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:radio.group label="Select registration type for the SLAS Officer"
            wire:model.live.debounce.150ms="slasRegType">
            <flux:radio name="slasRegType" value="new" disabled label="New SLAS Officer"
                description="New SLAS Officer users can perform any action." />
            <flux:radio name="slasRegType" value="existing" label="Existing SLAS Officer"
                description="Existing SLAS Officer users have the ability to read, create, and update." />
        </flux:radio.group>

        @if ($slasRegType != 'new')
        <div class=" border border-red-500 bg-red-100 rounded-lg p-2">
            <p class=" text-red font">නවක පරිපාලන නිළදාරියකු ලෙස පත්වීමක් ලබා නොගන්නා අවස්තාවක, සේවා කාලයක්
                සහිත පරිපාලන නිළදාරියකු ලියාපදිංචි කිරීම සඳහා පමණි
            <div class="br"></div>Only for the registration of a SLAS Officer with a period of service,
            in the event that an appointment is not obtained as a new SLAS Officer.</p>
        </div>
        <div class=" space-y-6">
            <div class="flex flex-col md:flex-row gap-4">
    ...
    <div class="flex justify-between mt-8 bg-gray-50 dark:bg-neutral-900 p-2 rounded-xl border border-gray-200 dark:border-neutral-700">
        ...
        @if ($step < $maxStep)
            ...
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">
                <span wire:loading.remove>Create SLAS Officer</span>
