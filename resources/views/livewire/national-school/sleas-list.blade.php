<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    ...
    <x-page-header
        ...
        :breadcrumbs="[
            ...
            'SLEAS Directory' => route('sleas.list')
        ]"
    >
        <x-slot:actions>
            <flux:modal.trigger name="search-profile">
                <flux:button variant="subtle" icon="magnifying-glass" class="h-11 bg-white! dark:bg-slate-900! shadow-sm">Search Officers...</flux:button>
            </flux:modal.trigger>
            ...
            @can('sleas.create')
            <flux:button href="{{ route('sleas.create') }}" icon="plus" class="h-11 bg-indigo-600! hover:bg-indigo-700! text-white! shadow-lg shadow-indigo-200/50 dark:shadow-none border-none">
                Add SLEAS Officer
            </flux:button>
            @endcan
        </x-slot:actions>
    ...
    <div class="space-y-4">
        ...
        <div class="group relative bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl p-5 hover:shadow-xl hover:border-indigo-200 flex flex-col lg:flex-row lg:items-center gap-6">
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto lg:ml-auto border-t lg:border-t-0 border-slate-200 dark:border-slate-700 pt-4 lg:pt-0">
                <flux:button href="{{ route('sleas.profile.index', $employee->id) }}" size="sm" class="w-full lg:w-auto rounded-xl! bg-indigo-600! dark:bg-white! text-white! dark:text-slate-900! font-bold px-4 hover:opacity-90">View</flux:button>
                <flux:dropdown>
        ...
        <div class="space-y-2 mt-4">
            ...
            <a href="{{ route('sleas.profile.index', $sleas->id) }}" class="flex items-center gap-4 p-4 rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-900/10 border border-transparent hover:border-indigo-100 dark:hover:border-indigo-800 transition-all">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-500">{{ $sleas->nic }}</p>
