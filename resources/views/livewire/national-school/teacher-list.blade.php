<div class="p-4 md:p-8 max-w-8xl mx-left min-h-screen">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        ...
        <div class="flex flex-wrap items-center gap-3">
            <flux:modal.trigger name="search-profile">
                <flux:button variant="subtle" icon="magnifying-glass" class="h-11 bg-white! dark:bg-slate-900! shadow-sm dark:shadow-none dark:border-slate-700">Search Teachers...</flux:button>
            </flux:modal.trigger>
            ...
            @can('teacher.create')
            <flux:button href="{{ route('teacher.create') }}" icon="plus" class="h-11 bg-indigo-600! hover:bg-indigo-700! dark:bg-indigo-500! dark:hover:bg-indigo-600! text-white! shadow-lg shadow-indigo-200 dark:shadow-indigo-900/20 border-none">
                Add Teacher
            </flux:button>
            @endcan
        </div>
    ...
    <div class="space-y-4">
        ...
        <div class="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:shadow-xl hover:border-indigo-200 flex flex-col lg:flex-row lg:items-center gap-6">
            ...
            <div class="flex items-center gap-2 lg:ml-auto">
                <flux:button href="{{ route('teacher.profile.index', $employee->id) }}" size="sm" class="rounded-xl! bg-slate-900! dark:bg-white! text-white! dark:text-slate-900! font-bold px-4 hover:opacity-90">View</flux:button>
                <flux:dropdown>
        ...
        <div class="py-20 flex flex-col items-center justify-center bg-white dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[3rem]">
            ...
            {{-- Action --}}
            @can('teacher.create')
            <flux:button href="{{ route('teacher.create') }}" icon="plus" class="h-11 bg-indigo-600! mt-6 hover:bg-indigo-700! dark:bg-indigo-500! dark:hover:bg-indigo-600! text-white! shadow-lg shadow-indigo-200 dark:shadow-indigo-900/20 border-none">
                Add New Teacher
            </flux:button>
            @endcan
        </div>
    ...
    <flux:modal name="search-profile" variant="flyout" class="space-y-6">
        ...
        <div class="space-y-2 mt-4">
            ...
            <a href="{{ route('teacher.profile.index', $teacher->id) }}" class="flex items-center gap-4 p-4 rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-900/10 border border-transparent hover:border-indigo-100 dark:hover:border-indigo-800 transition-all">
                ...
                <div>
                    <p class="font-bold text-slate-900 dark:text-white leading-tight">{{ $teacher->name_with_initials }}</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ $teacher->nic }}</p>
                    <p class="text-xs text-indigo-400 dark:text-indigo-300">{{ $teacher->currentAppointment?->workplace?->office()?->name }}</p>
                </div>
            </a>
            @empty
