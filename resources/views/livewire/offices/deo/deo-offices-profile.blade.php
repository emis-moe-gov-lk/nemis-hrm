<div class="antialiased min-h-screen">
    ...
    <div class="max-w-5xl bg-white dark:bg-gray-800 p-6 rounded-b-lg">
        ...
        <div class="flex items-center space-x-4 mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
            <div
                class="relative w-20 h-20 sm:w-24 sm:h-24 rounded-lg overflow-hidden border-2 {{ $divisionalEducationOffice->active_status ? 'border-green-500' : 'border-red-500' }}">

                @if ($divisionalEducationOffice->logo)
                    <img src="{{ asset('storage/' . $divisionalEducationOffice->logo) }}"
                        alt="{{ $divisionalEducationOffice->name }}"
                        class="w-full h-full object-cover {{ $divisionalEducationOffice->active_status ? '' : 'grayscale opacity-60' }}">
                    @unless ($divisionalEducationOffice->active_status)
                        <div
            ...
            <div>
                <h1 class="text-xl font-bold text-gray-900 dark:text-white leading-tight">
                    {{ $divisionalEducationOffice->name }}
                </h1>
                @if ($divisionalEducationOffice->short_name)
                    <p class="text-sm text-gray-600 dark:text-gray-400">
                        {{ $divisionalEducationOffice->short_name }}
                    </p>
                @endif
            </div>
