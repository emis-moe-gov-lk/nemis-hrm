<section class="w-full">
    ...
    <div class="relative mb-8 w-full">
        <flux:heading size="xl" level="1" class="mb-1">{{ __('Institution Cadre Summary for DEO') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('List of Cadre Summary under the selected DEO.') }}</flux:subheading>
    ...
    <x-offices.deo.deo-layout :officeId="$officeId">
        ...
        <div class="flex justify-end w-full mb-6">
            <div class="flex items-center gap-3 px-4 py-3 bg-white border border-slate-200 rounded-2xl shadow-sm w-full sm:w-fit">
                <div class="flex-1 sm:flex-none text-right">
                    <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5">{{ __('Circular') }}</span>
                ...
                <div class="flex-1 sm:flex-none text-right">
                    <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5">{{ __('Date') }}</span>
        ...
        <div class="md:hidden space-y-12">
            @forelse($groupedRows as $typeId => $items)
                ...
                <div class="space-y-4">
                    ...
                    @foreach($items as $row)
                        <div class="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm active:scale-[0.98] transition-transform">
                            <div class="p-5 border-b border-slate-50 flex justify-between items-start gap-4">
                                <div class="flex-1">
                            ...
                            <div class="px-5 py-4 grid grid-cols-3 gap-4 bg-slate-50/30 text-center">
                                <div class="space-y-1">
                                    <dt class="text-[9px] uppercase font-bold text-slate-400 tracking-tighter">{{ __('Approved') }}</dt>
                                    <dd class="text-base font-extrabold text-slate-700">{{ $row['approved_posts'] }}</dd>
                                </div>
                                <div class="space-y-1">
                                    <dt class="text-[9px] uppercase font-bold text-slate-400 tracking-tighter">{{ __('Filled') }}</dt>
                                    <dd class="text-base font-extrabold text-slate-700">{{ $row['filled_posts'] }}</dd>
                                </div>
                                <div class="space-y-1">
                                    <dt class="text-[9px] uppercase font-bold text-slate-400 tracking-tighter">{{ __('Gap') }}</dt>
            ...
            @empty
                <div class="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-slate-200 text-slate-400 text-sm">
                    <div class="mb-2">📭</div>
            ...
            {{-- MOBILE GRAND TOTAL --}}
            @if($rows)
            <div class="p-6 bg-slate-900 rounded-[2.5rem] text-white shadow-2xl mt-10 relative overflow-hidden">
        ...
        <div class="hidden md:block bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden mt-6">
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/80 border-b border-slate-200">
                    <tr>
                        <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 border-r border-slate-100">Subject / Medium</th>
                        <th class="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-right">Approved</th>
                        <th class="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-right">Filled</th>
                        <th class="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-right">Gap</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($groupedRows as $typeId => $items)
                        ...
                        <tr class="{{ $currentType['class'] }} border-y border-slate-100/50">
                        ...
                        @foreach($items as $row)
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                <td class="px-6 py-4 border-r border-slate-50">
                                    ...
                                    <div class="text-[10px] text-slate-400 font-bold uppercase mt-1 tracking-tight">{{ $row['medium_name'] }}</div>
                                </td>
                ...
                @if($rows)
                <tfoot class="bg-slate-900 text-white border-t-4 border-slate-800">
                    <tr>
                        ...
                        <td class="px-4 py-8 text-right text-2xl font-black text-slate-400 tracking-tighter">
                            {{ $grandFilled }}
                        </td>
