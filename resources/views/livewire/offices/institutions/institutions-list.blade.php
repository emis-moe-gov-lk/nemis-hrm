<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    ...
    <x-page-header
        title="Institutions"
        subtitle="Manage school profiles, census data, and localized accounts"
        icon="academic-cap"
        :count="$institutions->total()"
        countLabel="Total Registered"
        :breadcrumbs="[
            'Administrative Hierarchy' => route('offices.index'),
            'Institutions' => route('offices.institutions.list')
        ]"
    >
        <x-slot:actions>
        ...
        <div class="group bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-4xl p-2 pr-6 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/5 hover:border-indigo-500/30">
            <div class="flex flex-col lg:flex-row lg:items-center gap-6">
                ...
                <div class="flex items-center gap-2 pl-4 lg:pl-0">
                    @can('office.institution.profile.overview.view')
                    <flux:button href="{{ route('offices.institutions.profile.overview', $institution->id) }}" variant="filled" size="sm" class="rounded-xl shadow-sm">
                        View
                    </flux:button>
                    @endcan
