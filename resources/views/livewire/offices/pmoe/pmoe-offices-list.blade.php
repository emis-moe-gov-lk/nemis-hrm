<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    <x-page-header
        title="Provincial Ministry Offices"
        subtitle="Manage Provincial Ministry of Education profiles and regional administration"
        icon="building-office-2"
        :breadcrumbs="[
            'Administrative Hierarchy' => route('offices.index'),
            'Provincial Ministry' => route('offices.pmoe.list')
        ]"
    >
        <x-slot:actions>
             @can('office.pmoe.create')
                <a href="{{ route('offices.pmoe.create') }}" wire:navigate>
    ...
    <div class="space-y-4">
        @forelse ($provincialEducationMinistries as $data)
            <div class="group bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-3xl p-2 pr-6 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/5 hover:border-indigo-500/30">
                <div class="flex flex-col lg:flex-row lg:items-center gap-6">
                    ...
                    <a href="{{route('offices.peo.by-pmoe', $data->id)}}" wire:navigate>
                    ...
                    <div class="flex items-center gap-2 pl-4 lg:pl-0">
                        @can('office.pmoe.profile.overview.view')
                            <flux:button href="{{ route('offices.pmoe.profile.overview', $data->id) }}" variant="filled" size="sm" class="rounded-xl">
                                View
                            </flux:button>
                        @endcan
        ...
        @empty
            <div class="flex flex-col items-center justify-center py-24 bg-slate-50 dark:bg-slate-800/50 rounded-[2.5rem] border-2 border-dashed border-slate-300 dark:border-slate-700">
                ...
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">No Provincial Offices found</h3>
                <p class="text-slate-500 text-sm mt-1">Try creating a new record or checking your permissions.</p>
            </div>
        @endforelse
    </div>
