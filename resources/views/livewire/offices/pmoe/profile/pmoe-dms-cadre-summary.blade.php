<section class="w-full">
    ...
    <div class="relative mb-8 w-full">
        <flux:heading size="xl" level="1" class="mb-1">{{ __('Institution Cadre Summary for PMOE') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('List of Cadre Summary under the selected PMOE.') }}</flux:subheading>
    ...
    <x-offices.pmoe.layout :officeid="$officeId">
        ...
        <div class="w-full mb-8 grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
            {{-- Total Approved Card --}}
            <div class="relative overflow-hidden bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] p-6 shadow-sm">
                <!-- Background Icon -->
                ...
                <div class="relative z-10 flex flex-col justify-center h-full">
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{{ __('Total Approved') }}</div>
            ...
            <div class="relative overflow-hidden bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] p-6 shadow-sm">
                ...
                <!-- Content -->
                <div class="relative z-10 flex flex-col justify-center h-full">
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{{ __('Total Filled') }}</div>
            ...
            <div class="relative overflow-hidden bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] p-6 shadow-sm">
                ...
                <!-- Content -->
                <div class="relative z-10 flex flex-col justify-center h-full">
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{{ __('Overall Gap') }}</div>
        ...
        <div class="md:hidden space-y-12">
            @forelse($groupedRows as $typeId => $items)
                @php $currentType = $typeLabels[$typeId] ?? ['label' => 'General', 'class' => 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-100 dark:border-slate-700']; @endphp
                
                <div class="space-y-4">
                    {{-- Category Sticky Header --}}
                    <div class="sticky top-0 z-10 py-2 bg-slate-50/95 dark:bg-[#0B1120]/95 backdrop-blur-sm -mx-4 px-4 mb-2 border-b border-transparent dark:border-slate-800/50">
                        ...
                        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm active:scale-[0.98] transition-transform">
                            <div class="p-5 border-b border-slate-50 dark:border-slate-800/50 flex justify-between items-start gap-4">
                                ...
                                @php
                                    ...
                                    ][$row['status']] ?? 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-700';
                                @endphp
        ...
        <div class="hidden md:block bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden mt-6">
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/80 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800">
                    <tr>
                        <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 border-r border-slate-100 dark:border-slate-700/50">Subject / Medium</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50">
                    @forelse($groupedRows as $typeId => $items)
                        ...
                        <tr class="{{ $currentType['class'] }} border-y border-slate-100/50 dark:border-slate-800/50">
