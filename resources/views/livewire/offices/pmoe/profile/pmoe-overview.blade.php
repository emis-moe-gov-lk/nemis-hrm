<section class="w-full">
    ...
    <div class="relative mb-8 w-full">
        <flux:heading size="xl" level="1" class="mb-1">{{ __('Provincial Ministry of Education Overview') }}</flux:heading>
    ...
    <x-offices.pmoe.layout :officeid="$officeId">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-6 mb-8 p-6 bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] shadow-sm">
            <div class="relative w-24 h-24 rounded-2xl overflow-hidden border-2 border-slate-100 dark:border-slate-800 shadow-inner bg-slate-50 dark:bg-slate-900 shrink-0">
                @if ($ProvincialMinistry->logo)
                <img src="{{ asset('storage/' . $ProvincialMinistry->logo) }}"
                    alt="{{ $ProvincialMinistry->name }}"
                    class="w-full h-full object-cover {{ $ProvincialMinistry->active_status ? '' : 'grayscale opacity-60' }}">
                @else
                ...
                @unless ($ProvincialMinistry->active_status)
            ...
            <div class="flex-1 text-center sm:text-left">
                <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight mb-1">
                    {{ $ProvincialMinistry->name }}
                </h1>
                @if ($ProvincialMinistry->short_name)
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
            ...
            <div class="relative overflow-hidden bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] p-6 shadow-sm group hover:border-blue-500/30 transition-all">
                ...
                <div class="relative z-10 flex flex-col justify-center h-full">
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{{ __('Students') }}</div>
                    <div class="text-4xl font-black text-slate-900 dark:text-white tracking-tighter">{{ number_format($studentCount) }}</div>
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight mt-1">{{ __('Estimated Total Students') }}</div>
                </div>
            </div>
            ...
            <div class="relative overflow-hidden bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] p-6 shadow-sm group hover:border-emerald-500/30 transition-all">
                ...
                <div class="relative z-10 flex flex-col justify-center h-full">
                    ...
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight mt-1">{{ __('Departments under PMOE') }}</div>
                </div>
            </div>
            ...
            <div class="relative overflow-hidden bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] p-6 shadow-sm group hover:border-amber-500/30 transition-all">
                ...
                <div class="relative z-10 flex flex-col justify-center h-full">
                    ...
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight mt-1">{{ __('Zones under Provinces') }}</div>
                </div>
            </div>
            ...
            {{-- Institutions Card --}}
            <div class="relative overflow-hidden bg-white dark:bg-[#151518] border border-slate-200 dark:border-slate-800/60 rounded-[1.25rem] p-6 shadow-sm group hover:border-cyan-500/30 transition-all">
                ...
                <div class="relative z-10 flex flex-col justify-center h-full">
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">{{ __('Institutions') }}</div>
                    <div class="text-4xl font-black text-slate-900 dark:text-white tracking-tighter">{{ $institutionCount }}</div>
                    <div class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-tight mt-1">{{ __('Schools under Divisions') }}</div>
                </div>
            </div>
        </div>
