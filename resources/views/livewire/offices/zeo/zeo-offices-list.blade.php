<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    <x-page-header
        title="Zonal Education Offices"
        subtitle="Manage Zonal profiles and regional administrative clusters"
    ...
    >
        <x-slot:actions>
             @can('office.zeo.create')
                <a href="{{ route('offices.zeo.create') }}" wire:navigate>
    ...
    <div class="space-y-4">
        @forelse ($zonalEducationOffices as $data)
            <div class="group bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-3xl p-2 pr-6 transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/5 hover:border-indigo-500/30">
                <div class="flex flex-col lg:flex-row lg:items-center gap-6">
                    ...
                    <a href="{{route('offices.deo.by-zone', $data->id)}}" wire:navigate>
                        <div class="flex items-center gap-5 p-3 lg:w-80 bg-slate-50 dark:bg-slate-800/50 rounded-2xl transition-all">
                    ...
                    <div class="flex-1 grid grid-cols-1 md:grid-cols-3 gap-8 px-4 lg:px-0">
                        {{-- Address Detail --}}
                        ...
                        {{-- Parent Office Affiliation --}}
                        <div class="space-y-1.5">
                            <div class="flex items-center gap-2 text-slate-500">
                                <flux:icon.building-library variant="micro" class="size-3.5" />
                                <span class="text-[10px] font-bold uppercase tracking-wider">Provincial Affiliation</span>
                            </div>
                            <p class="text-sm font-semibold text-slate-700 dark:text-slate-200">
                                {{ $data->provincialEducationOffice->short_name ?? 'N/A' }}
                            </p>
                        </div>
        ...
        @empty
            <div class="flex flex-col items-center justify-center py-24 bg-slate-50 dark:bg-slate-800/50 rounded-[2.5rem] border-2 border-dashed border-slate-300 dark:border-slate-700">
                ...
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">No Zonal Offices found</h3>
                <p class="text-slate-500 text-sm mt-1">Adjust your province filters or create a new record.</p>
            </div>
        @endforelse
    </div>
