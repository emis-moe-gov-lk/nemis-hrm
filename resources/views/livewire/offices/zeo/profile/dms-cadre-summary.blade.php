<section class="w-full">
    ...
    <div class="relative mb-8 w-full">
        ...
        <flux:subheading size="lg" class="mb-6 text-slate-500 dark:text-slate-400">{{ __('List of Cadre Summary under the selected ZEO.') }}</flux:subheading>
    ...
    <x-offices.zeo.zeo-layout :officeId="$officeId">
        ...
        <div class="flex justify-end w-full mb-6">
            <div class="flex items-center gap-3 px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm w-full sm:w-fit">
                <div class="flex-1 sm:flex-none text-right">
                    <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-0.5">{{ __('Circular') }}</span>
                ...
                <div class="flex-1 sm:flex-none text-right">
                    <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-0.5">{{ __('Date') }}</span>
        ...
        <div class="md:hidden space-y-12">
            ...
            @php $currentType = $typeLabels[$typeId] ?? ['label' => 'General', 'class' => 'bg-slate-50 text-slate-700 border-slate-100 dark:bg-slate-800/50 dark:text-slate-300 dark:border-slate-700']; @endphp
            ...
            <div class="space-y-4">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm active:scale-[0.98] transition-transform">
                    <div class="p-5 border-b border-slate-50 dark:border-slate-800/50 flex justify-between items-start gap-4">
                        <div class="flex-1">
                            <div class="text-base font-bold text-slate-900 dark:text-white leading-tight mb-1">{{ $row['subject_name'] }}</div>
                            <div class="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-tight">{{ $row['medium_name'] }}</div>
                        </div>
                        ...
                        ][$row['status']] ?? 'bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700';
                        @endphp
                    ...
                    <div class="px-5 py-4 grid grid-cols-3 gap-4 bg-slate-50/30 dark:bg-slate-800/30 text-center">
                        <div class="space-y-1">
                            <dt class="text-[9px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-tighter">{{ __('Approved') }}</dt>
                            <dd class="text-base font-extrabold text-slate-700 dark:text-slate-300">{{ $row['approved_posts'] }}</dd>
                        </div>
                        <div class="space-y-1">
                            <dt class="text-[9px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-tighter">{{ __('Filled') }}</dt>
                            <dd class="text-base font-extrabold text-slate-700 dark:text-slate-300">{{ $row['filled_posts'] }}</dd>
                        </div>
                        <div class="space-y-1">
                            <dt class="text-[9px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-tighter">{{ __('Gap') }}</dt>
            ...
            @empty
            <div class="text-center py-16 bg-white dark:bg-slate-900 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 text-sm">
            ...
            {{-- MOBILE GRAND TOTAL --}}
            @if($rows)
            <div class="p-6 bg-slate-900 dark:bg-slate-950/80 rounded-[2.5rem] text-white shadow-2xl mt-10 relative overflow-hidden ring-1 ring-white/10 dark:ring-white/5">
                <div class="absolute top-0 right-0 p-4 opacity-10 text-4xl font-black"></div>
                <div class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400 mb-6 border-b border-slate-800 dark:border-slate-800/50 pb-3">{{ __('Grand Summary') }}</div>
                <div class="grid grid-cols-3 gap-6 text-center relative z-10">
                    <div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">{{ __('Approved') }}</div>
                        <div class="text-2xl font-black tracking-tighter text-white">{{ $grandApproved }}</div>
                    </div>
                    <div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">{{ __('Filled') }}</div>
                        <div class="text-2xl font-black text-slate-300 dark:text-slate-400 tracking-tighter">{{ $grandFilled }}</div>
                    </div>
                    <div>
                        <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">{{ __('Gap') }}</div>
        ...
        <div class="hidden md:block bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden mt-6">
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50/80 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                    <tr>
                        <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 border-r border-slate-100 dark:border-slate-800/50">Subject / Medium</th>
                        <th class="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 text-right">Approved</th>
                        <th class="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 text-right">Filled</th>
                        <th class="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 text-right">Gap</th>
                        <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 text-center">Status</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    ...
                    <tr class="{{ $currentType['class'] }} border-y border-slate-100/50 dark:border-slate-800/30">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                        <td class="px-6 py-4 border-r border-slate-50 dark:border-slate-800/50">
                            ...
                            <div class="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase mt-1 tracking-tight">{{ $row['medium_name'] }}</div>
                        </td>
                        <td class="px-4 py-4 text-right text-sm font-bold text-slate-600 dark:text-slate-400 tracking-tight">{{ $row['approved_posts'] }}</td>
                        <td class="px-4 py-4 text-right text-sm font-bold text-slate-600 dark:text-slate-400 tracking-tight">{{ $row['filled_posts'] }}</td>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-20 text-center text-slate-400 dark:text-slate-500 italic text-sm">
                            <div class="mb-2 opacity-50 dark:opacity-40 text-4xl">📭</div>
                ...
                @if($rows)
                <tfoot class="bg-slate-900 dark:bg-slate-950/80 text-white border-t-4 border-slate-800 dark:border-slate-900">
                    <tr>
                        <td class="px-6 py-8 text-xs font-black uppercase tracking-[0.3em] text-slate-500 dark:text-slate-400">
                            {{ __('Grand Summary') }}
                        </td>
                        <td class="px-4 py-8 text-right text-2xl font-black tracking-tighter text-white">
                            {{ $grandApproved }}
                        </td>
                        <td class="px-4 py-8 text-right text-2xl font-black text-slate-400 dark:text-slate-500 tracking-tighter">
                            {{ $grandFilled }}
                        </td>
                        ...
                        <td class="px-6 py-8 text-center">
                            <span class="px-3 py-1.5 rounded-lg bg-slate-800 dark:bg-slate-800/80 text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest border border-slate-700 dark:border-slate-700/50">
                                {{ __('Finalized') }}
                            </span>
                        </td>
                    </tr>
