<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Create Education Administrator Officer') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Create Education Administrator Officer profile and account') }}</flux:subheading>
...
<form wire:submit.prevent="save" class="mt-6 max-w-2xl space-y-6">
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Recruitment Category" wire:model.live.debounce.150ms="recruitmentCategory">
                        <option value="">Select</option>
                        @foreach ($recruitmentCategoryOption as $category)
            ...
            <div class="md:w-1/2 w-full">
                <flux:field>
                    <flux:select label="Recruitment Subject" wire:model.live.debounce.150ms="recruitmentSubject">
                        <option value="">Select</option>
                        @foreach ($recruitmentSubjectOption as $subject)
                        <option value="{{ $subject->eas_subject_id }}">{{ $subject->subject }}</option>
                        @endforeach
                    </flux:select>
                </flux:field>
            </div>
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:radio.group label="Select registration type for the Education Administrator Officer"
            wire:model.live.debounce.150ms="sleasRegType">
            <flux:radio name="sleasRegType" value="new" disabled label="New Education Administrator Officer"
                description="New Education Administrator Officer users can perform any action." />
            <flux:radio name="sleasRegType" value="existing" label="Existing Education Administrator Officer"
                description="Existing Education Administrator Officer users have the ability to read, create, and update." />
        </flux:radio.group>

        @if ($sleasRegType != 'new')
        <div class=" border border-red-500 bg-red-100 rounded-lg p-2">
            <p class=" text-red font">නවක අධ්‍යාපන අධ්‍යක්ෂවරයකු ලෙස පත්වීමක් ලබා නොගන්නා අවස්තාවක, සේවා කාලයක්
                සහිත අධ්‍යාපන අධ්‍යක්ෂවරයකු ලියාපදිංචි කිරීම සඳහා පමණි
            <div class="br"></div>Only for the registration of a Education Administrator Officer with a period of service,
            in the event that an appointment is not obtained as a new Education Administrator Officer.</p>
        </div>
        <div class=" space-y-6">
            <div class="flex flex-col md:flex-row gap-4">
            ...
            <flux:field>
                <flux:select label="Current Position" wire:model.live.debounce.150ms="currentPosition">
                    <option value="">Select</option>
    ...
    <div class="flex justify-between mt-8 bg-gray-50 dark:bg-neutral-900 p-2 rounded-xl border border-gray-200 dark:border-neutral-700">
        ...
        @if ($step < $maxStep)
            ...
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">
                <span wire:loading.remove>Create Education Administrator</span>
