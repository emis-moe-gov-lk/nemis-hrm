<div class="p-4 md:p-8 max-w-7xl mx-auto min-h-screen">
    {{-- Header Section --}}
    <x-page-header
        title="Teacher Advisor Service Directory"
        subtitle="Manage Sri Lanka Teacher Advisor Service profiles and registry."
        icon="academic-cap"
        :count="$employees->total()"
        countLabel="Profiles Registered"
        :breadcrumbs="[
            'Registry' => '#',
            'SLTAS' => route('sltas.list')
        ]">
        <x-slot:actions>
            @can('sltas.bulk.upload')
            ...
            @can('sltas.create')
            <flux:button href="{{ route('sltas.create') }}" icon="plus" class="h-11 bg-indigo-600! hover:bg-indigo-700! text-white! shadow-lg shadow-indigo-200 border-none">
                Add Teacher Advisor Officer
            </flux:button>
            @endcan
        </x-slot:actions>
    </x-page-header>
        ...
        <div class="w-full xl:w-auto">
            <flux:modal.trigger name="search-profile">
                <flux:button variant="filled" icon="magnifying-glass" class="w-full sm:w-auto h-10 bg-white! dark:bg-slate-900! text-slate-700! dark:text-slate-300! border border-slate-300 dark:border-slate-700 shadow-sm hover:border-indigo-300! transition-all group">
                    <span class="group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">Search Teacher Advisor Officers...</span>
                </flux:button>
            </flux:modal.trigger>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="group relative bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl p-5 transition-all duration-300 hover:shadow-xl hover:border-indigo-200 flex flex-col lg:flex-row lg:items-center gap-6">
            ...
            <div class="flex items-center gap-2 w-full lg:w-auto lg:ml-auto border-t lg:border-t-0 border-slate-200 dark:border-slate-700 pt-4 lg:pt-0">
                <flux:button href="{{ route('sltas.profile.index', $employee->id) }}" size="sm" class="w-full lg:w-auto rounded-xl! bg-indigo-600! text-white! font-bold px-4">View</flux:button>
                <flux:dropdown>
                    <flux:button icon="ellipsis-vertical" size="sm" variant="ghost" />
                    <flux:menu>
                        @can('sltas.profile.id.view')
                        <flux:menu.item href="{{ route('teacher.id.pdf', $employee->id) }}" download icon="identification">Print ID</flux:menu.item>
                        @endcan
                        @can('sltas.profile.pdf.view')
        ...
        <div class="py-20 flex flex-col items-center justify-center bg-white dark:bg-slate-900 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-[3rem] transition-all">
            ...
            {{-- Text Content --}}
            <h3 class="text-xl font-bold text-slate-800 dark:text-slate-200">No Teacher Advisor Officers Found</h3>
            ...
            {{-- Action --}}
            @can('sltas.create')
            <flux:button href="{{ route('sltas.create') }}" icon="plus" class="h-11 bg-indigo-600! mt-6 hover:bg-indigo-700! text-white! shadow-lg shadow-indigo-200 border-none">
                Add New Teacher Advisor Officer
            </flux:button>
            @endcan
        </div>
        @endforelse
    ...
    <flux:modal name="search-profile" variant="flyout" class="space-y-6">
        ...
        <div class="space-y-2 mt-4">
            @if(!empty($query))
            @forelse($results as $sltas)
            <a href="{{ route('sltas.profile.index', $sltas->id) }}" class="flex items-center gap-4 p-4 rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-900/10 border border-transparent hover:border-indigo-100 transition-all">
                <div class="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center font-bold">
                    {{ substr($sltas->name_with_initials, 0, 1) }}
                </div>
                <div>
                    <p class="font-bold text-slate-900 dark:text-white leading-tight">{{ $sltas->name_with_initials }}</p>
                    <p class="text-xs text-slate-500">{{ $sltas->nic }}</p>
                    <p class="text-xs text-indigo-400">{{ $sltas->currentAppointment?->workplace?->office_name ?? 'N/A' }}</p>
                </div>
            </a>
            @empty
