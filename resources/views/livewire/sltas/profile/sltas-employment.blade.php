            ...
            <div class="antialiased min-h-screen">
                ...
                <div class="bg-white dark:bg-gray-800 p-2 rounded-b-lg">
                    ...
                    <div class="space-y-6">
                        ...
                        <livewire:employees.appointment-current-status :peopleId="$people->people_id" :canEdit="auth()->user()->can('sltas.profile.employment.current-appointment.update')" />

                        {{-- 2. First Employment --}}
                        <livewire:employees.first-appointment :peopleId="$people->people_id" :canEdit="auth()->user()->can('sltas.profile.employment.first-appointment.update')" />
                        <livewire:sltas.profile.sltas-data :peopleId="$people->people_id" />

                        {{-- 3. Previous Services --}}
                        <livewire:employees.previous-services-reg :peopleId="$people->people_id"
                            :canCreate="auth()->user()->can('sltas.profile.employment.previous-service.create')"
                            :canDelete="auth()->user()->can('sltas.profile.employment.previous-service.delete')" />

                        {{-- 4. Working Place History --}}
                        <livewire:employees.working-place-history :peopleId="$people->people_id"
                            :canCreate="auth()->user()->can('sltas.profile.employment.working-place-history.create')"
                            :canDelete="auth()->user()->can('sltas.profile.employment.working-place-history.delete')" />

                        {{-- 5. Position History --}}
                        <livewire:employees.position-history :peopleId="$people->people_id"
                            :canCreate="auth()->user()->can('sltas.profile.employment.position-history.create')"
                            :canDelete="auth()->user()->can('sltas.profile.employment.position-history.delete')" />
