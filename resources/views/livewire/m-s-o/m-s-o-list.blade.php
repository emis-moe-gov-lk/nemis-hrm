    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div>
            <flux:heading size="xl" class="text-3xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-indigo-600 to-blue-500">
                Management Assistant Officer Directory
            </flux:heading>
        ...
        <div class="flex flex-wrap items-center gap-3">
            <flux:modal.trigger name="search-profile">
                <flux:button variant="subtle" icon="magnifying-glass" class="h-11 !bg-white dark:!bg-slate-900 shadow-sm">Search MSO officers...</flux:button>
            </flux:modal.trigger>
    ...
    <div class="space-y-4">
        @forelse($employees as $employee)
            <div class="group relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 transition-all duration-300 hover:shadow-xl hover:border-indigo-200 flex flex-col lg:flex-row lg:items-center gap-6">
                ...
                <div class="flex items-center gap-2 lg:ml-auto">
                    <flux:button href="{{ route('mso.profile.index', $employee->id) }}" size="sm" class="!rounded-xl !bg-slate-900 !text-white font-bold px-4">View</flux:button>
                    <flux:dropdown>
        ...
        @empty
            <div class="py-20 flex flex-col items-center justify-center bg-white dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-[3rem] transition-all">
                ...
                {{-- Text Content --}}
                <h3 class="text-xl font-bold text-slate-800 dark:text-slate-200">No msos Found</h3>
                ...
                @forelse($results as $mso)
                    <a href="{{ route('mso.profile.index', $mso->id) }}" class="flex items-center gap-4 p-4 rounded-2xl hover:bg-indigo-50 dark:hover:bg-indigo-900/10 border border-transparent hover:border-indigo-100 transition-all">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-indigo-400">{{ $mso->currentAppointment?->workplace?->office()?->name }}</p>
                        </div>
                    </a>
                @empty
