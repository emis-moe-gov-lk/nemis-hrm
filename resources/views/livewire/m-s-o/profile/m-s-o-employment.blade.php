            ...
            <div class="antialiased min-h-screen">
                ...
                <div class="bg-white dark:bg-gray-800 p-2 rounded-b-lg">
                    ...
                    <div class="space-y-6">
                        ...
                        {{-- 2. First Employment --}}
                        <livewire:employees.first-appointment :peopleId="$people->people_id" :canEdit="auth()->user()->can('mso.profile.employment.first-appointment.update')"/>
                        <livewire:m-s-o.profile.m-s-o-data :peopleId="$people->people_id" />

                        {{-- 3. Previous Services --}}
                        <livewire:employees.previous-services-reg :peopleId="$people->people_id" :canCreate="auth()->user()->can('mso.profile.employment.previous-service.create')" :canDelete="auth()->user()->can('mso.profile.employment.previous-service.delete')"/>
                            
                        {{-- 4. Employment Service History --}}
                        <livewire:employees.services-history :peopleId="$people->people_id" :canCreate="auth()->user()->can('mso.profile.employment.services-history.create')" :canDelete="auth()->user()->can('mso.profile.employment.services-history.delete')"/>
