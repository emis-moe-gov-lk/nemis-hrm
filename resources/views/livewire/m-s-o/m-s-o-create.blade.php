<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Create Management Assistant Officer') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Create Management Assistant Officer profile and account') }}</flux:subheading>
...
<form wire:submit.prevent="save" class="mt-6 max-w-2xl space-y-6">
    ...
    <div class="mt-6 max-w-2xl space-y-6">
        ...
        <flux:radio.group label="Select registration type for the Management Assistant Officer"
            wire:model.live.debounce.150ms="msoRegType">
            <flux:radio name="msoRegType" value="new" disabled label="New Management Assistant Officer"
                description="New Management Assistant Officer users can perform any action." />
            <flux:radio name="msoRegType" value="existing" label="Existing Management Assistant Officer"
                description="Existing Management Assistant Officer users have the ability to read, create, and update." />
        </flux:radio.group>

        @if ($msoRegType != 'new')
        <div class=" border border-red-500 bg-red-100 rounded-lg p-2">
            <p class=" text-red font">නවක කළමණාකරණ සහකාරවරයකු ලෙස පත්වීමක් ලබා නොගන්නා අවස්තාවක, සේවා කාලයක්
                සහිත කළමණාකරණ සහකාරවරයකු ලියාපදිංචි කිරීම සඳහා පමණි
            <div class="br"></div>Only for the registration of a Management Assistant Officer with a period of service,
            in the event that an appointment is not obtained as a new Management Assistant Officer.</p>
        </div>
        <div class=" space-y-6">
            <div class="flex flex-col md:flex-row gap-4">
    ...
    <div class="flex justify-between mt-8 bg-gray-50 dark:bg-neutral-900 p-2 rounded-xl border border-gray-200 dark:border-neutral-700">
        ...
        @if ($step < $maxStep)
            ...
            <flux:button type="submit" variant="primary" wire:loading.attr="disabled">
                <span wire:loading.remove>Create Management Assistant</span>
