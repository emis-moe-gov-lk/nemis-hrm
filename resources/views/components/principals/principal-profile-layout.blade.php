<div class="max-w-7xl mx-left pb-10 px-0 md:px-4">
        ...
        <div class="px-4 md:px-8 pb-2">
            <div class="relative flex flex-col md:flex-row items-center md:items-end -mt-12 md:-mt-16 mb-4 gap-4">
                ...
                <div class="relative">
                    ...
                    @endif
                    <span class="absolute bottom-2 right-2 md:bottom-3 md:right-3 block h-4 w-4 md:h-5 md:w-5 rounded-full border-2 border-white dark:border-gray-900 {{ $principal->appointment->is_confirmed ? 'bg-green-500' : 'bg-red-500' }}"></span>
                </div>
                ...
                <div class="flex-1 text-center md:text-left md:mb-4">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 mt-1">
                        <p class="text-indigo-600 dark:text-indigo-400 font-bold text-xs md:text-sm uppercase tracking-widest">
                            {{ $principal->currentAppointment->appointment?->service?->service_name ?? 'N/A' }}
                        </p>
                        ...
                        <p class="text-slate-500 dark:text-slate-500 font-medium text-sm md:text-base flex items-center justify-center md:justify-start gap-1.5">
                            ...
                            {{ $principal->currentAppointment?->workplace?->office_name ?? 'No Workplace Assigned' ?? 'No Workplace Assigned' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="flex gap-2 mb-4 w-full md:w-auto justify-center px-4 md:px-0">
                    @can('principal.profile.edit-request.create')
                    ...
                    <a href="{{ route('teacher.profile.pdf', $principal->id) }}" download class="flex-1 md:flex-none">
            ...
            <div class="w-full mt-2">
                <nav class="flex justify-between md:justify-start gap-1 md:gap-5 p-1 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                    {{-- General --}}
                    <a href="{{ route('principal.profile.index', $principalid) }}" wire:navigate 
                    ...
                    {{ request()->routeIs('principal.profile.index') 
                    ...
                    {{-- Qualification --}}
                    @can('principal.profile.qualification.view')
                    <a href="{{ route('principal.profile.qualification', $principalid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('principal.profile.qualification') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- Employment --}}
                    @can('principal.profile.employment.view')
                    <a href="{{ route('principal.profile.employment', $principalid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('principal.profile.employment') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- W&OP --}}
                    @can('principal.profile.pension-and-payment.view')
                    <a href="{{ route('principal.profile.pension-and-payment', $principalid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('principal.profile.pension-and-payment') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- Family --}}
                    @can('principal.profile.family.view')
                    <a href="{{ route('principal.profile.family', $principalid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('principal.profile.family') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- Edit Request --}}
                    @can('principal.profile.edit-request.view')
                    <a href="{{ route('principal.profile.edit-request', $principalid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('principal.profile.edit-request') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
    ...
    <div class="mt-4 md:mt-6 flex flex-col lg:flex-row gap-6 md:px-0">
            ...
            <div class="bg-white dark:bg-gray-900 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-1 gap-y-6 gap-x-4">
                    <div>
                        ...
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $principal->nic }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase font-bold text-gray-400">Employee ID</p>
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $principal->people_id }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase font-bold text-gray-400">W&OP No</p>
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $principal->appointment->w_op_no ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase font-bold text-gray-400">Pay Sheet No</p>
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $principal->appointment->pay_sheet_no ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="order-2 lg:order-1 flex-1 min-w-0">
            ...
            @if (!$principal->appointment?->is_confirmed && !$principal->appointment?->is_verified)
                @can('principal.profile.verify')
                <div class="mb-6">
                ...
                @endcan
            @elseif(!$principal->appointment?->is_confirmed && $principal->appointment?->is_verified)
                @can('principal.profile.confirm')
                <div class="mb-6">
                    <x-alert type="warning" :dismissible="false">
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div class="flex flex-col gap-1">
            ...
            @if ($principal->profileEditRequests->where('status', 1)->isNotEmpty())
                @can('principal.profile.edit-request.response')
                <div class="mb-6">
    ...
    <flux:modal name="edit-profile" class="md:w-150">
        <livewire:principal.complaint-form :peopleId="$principal->people_id" />
    </flux:modal>
    @endcan
    ...
    <flux:modal name="confirm-profile" class="md:w-96">
        <div class="space-y-6">
            <livewire:employees.confirm-profile-form :peopleId="$principal->people_id" />
        </div>
    </flux:modal>
    @endcan
    ...
    <flux:modal name="verify-profile" class="md:w-96">
        <div class="space-y-6">
            <livewire:employees.verify-profile-form :peopleId="$principal->people_id" />
        </div>
    </flux:modal>
    @endcan
    ...
    <flux:modal name="edit-profile-request-response" class="md:w-150">
        <livewire:employees.edit-profile-requst-response-form :peopleId="$principal->people_id" />
    </flux:modal>
    @endcan
</div>