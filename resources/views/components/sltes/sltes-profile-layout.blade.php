<div class="max-w-7xl mx-left pb-10 px-0 md:px-4">
        ...
        <div class="px-4 md:px-8 pb-2">
            <div class="relative flex flex-col md:flex-row items-center md:items-end -mt-12 md:-mt-16 mb-4 gap-4">
                ...
                <div class="relative">
                    ...
                    @endif
                    <span class="absolute bottom-2 right-2 md:bottom-3 md:right-3 block h-4 w-4 md:h-5 md:w-5 rounded-full border-2 border-white dark:border-gray-900 {{ $sltes->appointment->is_confirmed ? 'bg-green-500' : 'bg-red-500' }}"></span>
                </div>
                ...
                <div class="flex-1 text-center md:text-left md:mb-4">
                    <h1 class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tighter">
                        {{ $sltes->title->title_name }} {{ $sltes->name_with_initials }}
                    </h1>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 mt-1">
                        <p class="text-indigo-600 dark:text-indigo-400 font-bold text-xs md:text-sm uppercase tracking-widest">
                            {{ $sltes->currentAppointment?->appointment?->service?->service_name ?? 'Service Not Assigned' }}
                        </p>
                        ...
                        <p class="text-slate-500 dark:text-slate-500 font-medium text-sm md:text-base flex items-center justify-center md:justify-start gap-1.5">
                            ...
                            {{ $sltes->currentAppointment?->workplace?->office_name ?? 'No Workplace Assigned' ?? 'No Workplace Assigned' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="flex gap-2 mb-4 w-full md:w-auto justify-center px-4 md:px-0">
                    @can('sltes.profile.edit-request.create')
                    ...
                    <a href="{{ route('teacher.profile.pdf', $sltes->id) }}" download class="flex-1 md:flex-none">
            ...
            <div class="w-full mt-2">
                <nav class="flex justify-between md:justify-start gap-1 md:gap-5 p-1 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                    {{-- General --}}
                    <a href="{{ route('sltes.profile.index', $sltesid) }}" wire:navigate 
                    ...
                    {{ request()->routeIs('sltes.profile.index') 
                    ...
                    {{-- Qualification --}}
                    @can('sltes.profile.qualification.view')
                    <a href="{{ route('sltes.profile.qualification', $sltesid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('sltes.profile.qualification') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- Employment --}}
                    @can('sltes.profile.employment.view')
                    <a href="{{ route('sltes.profile.employment', $sltesid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('sltes.profile.employment') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- W&OP --}}
                    @can('sltes.profile.pension-and-payment.view')
                    <a href="{{ route('sltes.profile.pension-and-payment', $sltesid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('sltes.profile.pension-and-payment') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- Family --}}
                    @can('sltes.profile.family.view')
                    <a href="{{ route('sltes.profile.family', $sltesid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('sltes.profile.family') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
                    ...
                    {{-- Edit Request --}}
                    @can('sltes.profile.edit-request.view')
                    <a href="{{ route('sltes.profile.edit-request', $sltesid) }}" wire:navigate 
                    class="flex flex-1 md:flex-none flex-col md:flex-row items-center justify-center py-2.5 px-3 rounded-lg text-sm font-bold transition-all duration-200 
                    {{ request()->routeIs('sltes.profile.edit-request') 
                            ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' 
                            : 'text-gray-500 hover:bg-gray-200/60 dark:hover:bg-gray-700/50 hover:text-gray-700 dark:hover:text-white' }}">
    ...
    <div class="mt-4 md:mt-6 flex flex-col lg:flex-row gap-6 md:px-0">
            ...
            <div class="bg-white dark:bg-gray-900 p-5 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-1 gap-y-6 gap-x-4">
                    <div>
                        ...
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $sltes->nic }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase font-bold text-gray-400">Employee ID</p>
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $sltes->people_id }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase font-bold text-gray-400">W&OP No</p>
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $sltes->appointment->w_op_no ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase font-bold text-gray-400">Pay Sheet No</p>
                        <p class="text-sm font-mono font-medium text-gray-900 dark:text-gray-200">{{ $sltes->appointment->pay_sheet_no ?? 'N/A' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="order-2 lg:order-1 flex-1 min-w-0">
            ...
            @if (!$sltes->appointment?->is_confirmed && !$sltes->appointment?->is_verified)
                @can('sltes.profile.verify')
                <div class="mb-6">
                ...
                @endcan
            @elseif(!$sltes->appointment?->is_confirmed && $sltes->appointment?->is_verified)
                @can('sltes.profile.confirm')
                <div class="mb-6">
                    <x-alert type="warning" :dismissible="false">
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div class="flex flex-col gap-1">
            ...
            @if ($sltes->profileEditRequests->where('status', 1)->isNotEmpty())
                @can('sltes.profile.edit-request.response')
                <div class="mb-6">
    ...
    <flux:modal name="edit-profile" class="md:w-150">
        <livewire:sltes.complaint-form :peopleId="$sltes->people_id" />
    </flux:modal>
    @endcan
    ...
    <flux:modal name="confirm-profile" class="md:w-96">
        <div class="space-y-6">
            <livewire:employees.confirm-profile-form :peopleId="$sltes->people_id" />
        </div>
    </flux:modal>
    @endcan
    ...
    <flux:modal name="verify-profile" class="md:w-96">
        <div class="space-y-6">
            <livewire:employees.verify-profile-form :peopleId="$sltes->people_id" />
        </div>
    </flux:modal>
    @endcan
    ...
    <flux:modal name="edit-profile-request-response" class="md:w-150">
        <livewire:employees.edit-profile-requst-response-form :peopleId="$sltes->people_id" />
    </flux:modal>
    @endcan
</div>