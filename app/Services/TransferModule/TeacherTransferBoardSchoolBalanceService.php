...
class TeacherTransferBoardSchoolBalanceService
{
    public function summary(TeacherTransferBoard $board): array
    {
        $board->loadMissing(['subjects', 'workplace', 'policy', 'category', 'transferSubCategory']);
        ...
        $institutions = Institution::active()
            ...
            ->when(
                $board->transferSubCategory?->code === TransferSubCategoryRules::CODE_NATIONAL_SCHOOL,
                fn ($query) => $query->national(),
                fn ($query) => $query->provincial()
            )
            ->orderBy('name')
            ->get()
            ->keyBy('workplace_id');
    ...
    protected function zoneIdsForBoard(TeacherTransferBoard $board): Collection
    {
        ...
        if ($board->bo_office_level_id === 'OLID002') {
            return ZonalEducationOffice::active()
    ...
    protected function childWorkplaceIds(TeacherTransferBoard $board): array
    {
    ...
    protected function applicantMap(TeacherTransferBoard $board, string $scopeWorkplaceId, array $childWorkplaceIds, array $institutionIds, array $subjectIds): Collection
    {
    ...
    protected function approvedTransferMovementMaps(TeacherTransferBoard $board, string $scopeWorkplaceId, array $childWorkplaceIds, array $institutionIds, array $subjectIds): array
    {
