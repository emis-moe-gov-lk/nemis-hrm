...
class TeacherProfileForTransferBoard extends Component
{
    public $application;
    ...
    public function prepareDecisionDraft(): void
    {
        ...
        $decision = $this->application->boardRecommendation;
    ...
    public function getSelectedTransferBoardProperty(): ?TransferBoard
    {
    ...
    {
        return $this->selectedTransferBoard
            ?: $this->application->boardRecommendation?->board;
    }
    ...
    protected function recommendedNeededSchoolZoneIds(TransferBoard $board, string $scopeWorkplaceId): Collection
    {
        if ($this->isZonalDecisionBoard($board)) {
    ...
    protected function applyBoardScopeToApplicationQuery(Builder $query, TransferBoard $board, string $scopeWorkplaceId, array $childWorkplaceIds): Builder
    {
        if ($this->isZonalDecisionBoard($board)) {
            return $query->where(function (Builder $scopeQuery) use ($scopeWorkplaceId, $childWorkplaceIds) {
                $scopeQuery->whereHas('preferences', function (Builder $preferenceQuery) use ($scopeWorkplaceId) {
                    $preferenceQuery->where('zeo_wp_id', $scopeWorkplaceId);
                })
    ...
    {
        return view('livewire.transfer-module.transfer-board.teacher-profile-for-transfer-board');
    }
}
