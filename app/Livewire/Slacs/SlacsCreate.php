...
class SlacsCreate extends Component
{
    ...
    // Current Appointment
    // -------------------------
    public $slacsRegType = 'existing', $currentAppointmentDate, $currentAppointmentLetterNo, $currentService, $currentServiceRank;
    ...
    protected function rulesForCurrentStep()
    {
        switch ($this->step) {
            ...
            case 5:
                return [
                    'slacsRegType' => 'required|string|in:new,existing',
    ...
    public function mount()
    {
        ...
        $this->servicesOption = Service::where('service_id', 'SER009')->active()->get();
    ...
    // -------------------------
    public function updatedSlacsRegType($value)
    {
        if ($value === 'new') {
            ...
            // Reset current appointment fields for existing SLAcS officer
            $this->reset([
                'currentAppointmentDate',
    ...
    public function save()
    {
        ...
        try {
            ...
            $currentOfficeLevelID = $this->slacsRegType === 'new' ? $this->firstOfficeLevel : $this->currentOfficeLevel;
            $currentWorkplaceId = $this->slacsRegType === 'new' ? $this->firstWorkingPlace : $this->currentWorkingPlace;
            $currentServiceId = $this->slacsRegType === 'new' ? $this->service : $this->currentService;
            $currentRankId = $this->slacsRegType === 'new' ? $this->serviceRank : $this->currentServiceRank;
            $currentAppointDate = $this->slacsRegType === 'new' ? $this->firstAppointmentDate : $this->currentAppointmentDate;
            $currentAppointmentLetterNo = $this->slacsRegType === 'new' ? $this->appointmentLetterNo : $this->currentAppointmentLetterNo;
            ...
            // Assign SLAcS role
            $user->assignRole('Accountancy Service');
            ...
            session()->flash('success', 'Accountancy Service (SLAcS) Officer created successfully! Default password: ' . $password);
            $this->resetForm();
            $this->dispatch('scroll-top');
            ...
            DB::rollBack();
            session()->flash('error', 'Database error: Unable to save Accountancy Service (SLAcS) Officer data. Duplicate entry');
            $this->dispatch('scroll-top');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Accountancy Service (SLAcS) Officer creation error: ' . $e->getMessage(), [
    ...
    {
        return view('livewire.slacs.slacs-create');
    }
}
