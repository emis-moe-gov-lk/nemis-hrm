...
class SleasCreate extends Component
{
    ...
    public $recruitmentCategory, $recruitmentSubject;

    // -------------------------
    // Current Appointment
    // -------------------------
    public $sleasRegType = 'existing', $currentAppointmentDate, $currentAppointmentLetterNo, $currentService, $currentServiceRank;
    ...
    public $recruitmentCategoryOption = [], $recruitmentSubjectOption = [];
    ...
    protected function rulesForCurrentStep()
    {
        switch ($this->step) {
            ...
            case 5:
                return [
                    'sleasRegType' => 'required|string|in:new,existing',
                    ...
                    'cadreMedium' => 'nullable|required_if:currentOfficeLevel,OLID006|string',
                    'cadreSubject' => 'nullable|required_if:currentOfficeLevel,OLID006|string',
                ];
    ...
    public function mount()
    {
        ...
        $this->servicesOption = Service::where('service_id', 'SER005')->active()->get();
    ...
    // -------------------------
    public function updatedSleasRegType($value)
    {
        if ($value === 'new') {
    ...
    public function save()
    {
        ...
        try {
            ...
            $appointment = EmployerAppointment::create([
                ...
                'position_id' => $this->position, // education administrator officer position
            ...
            if ($this->cadreMedium === 'null') {
                $this->cadreMedium = null;
            }

            if ($this->cadreSubject === 'null') {
                $this->cadreSubject = null;
            }

            // Create Employer Cadre Subject
            if($this->cadreMedium !== null && $this->cadreSubject !== null) {
            ...
            // Create sleas record
            EducationAdministratorService::create([
                'appointment_id' => $appointment->appointment_id,
                'employee_id' => $people->people_id,
                'category_id' => $this->recruitmentCategory,
                'subject' => $this->recruitmentSubject,
            ]);

            // Determine current appointment values based on registration type
            $currentOfficeLevelID = $this->sleasRegType === 'new' ? $this->firstOfficeLevel : $this->currentOfficeLevel;
            $currentWorkplaceId = $this->sleasRegType === 'new' ? $this->firstWorkingPlace : $this->currentWorkingPlace;
            $currentServiceId = $this->sleasRegType === 'new' ? $this->service : $this->currentService;
            $currentRankId = $this->sleasRegType === 'new' ? $this->serviceRank : $this->currentServiceRank;
            $currentAppointDate = $this->sleasRegType === 'new' ? $this->firstAppointmentDate : $this->currentAppointmentDate;
            $currentAppointmentLetterNo = $this->sleasRegType === 'new' ? $this->appointmentLetterNo : $this->currentAppointmentLetterNo;
            ...
            EmployerCurrentAppointment::create([
                ...
                'position_id' => $this->currentPosition, // education administrator officer position
            ...
            // Assign education administrator officer role
            $user->assignRole('sleas officer');
            ...
            session()->flash('success', 'Education Administrator Officer created successfully! Default password: ' . $password);
            $this->resetForm();
            $this->dispatch('scroll-top');
            ...
            DB::rollBack();
            session()->flash('error', 'Database error: Unable to save Education Administrator Officer data.');
            $this->dispatch('scroll-top');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Education Administrator Officer creation error: ' . $e->getMessage(), [
