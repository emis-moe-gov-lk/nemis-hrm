...
class MSOCreate extends Component
{
    ...
    // Current Appointment
    // -------------------------
    public $msoRegType = 'existing', $currentAppointmentDate, $currentAppointmentLetterNo, $currentService, $currentServiceRank;
    ...
    protected function rulesForCurrentStep()
    {
        switch ($this->step) {
            ...
            case 5:
                return [
                    'msoRegType' => 'required|string|in:new,existing',
    ...
    public function mount()
    {
        ...
        $this->servicesOption = Service::where('service_id', 'SER008')->active()->get();
    ...
    // -------------------------
    public function updatedMSORegType($value)
    {
        if ($value === 'new') {
    ...
    public function save()
    {
        ...
        try {
            ...
            $currentOfficeLevelID = $this->msoRegType === 'new' ? $this->firstOfficeLevel : $this->currentOfficeLevel;
            $currentWorkplaceId = $this->msoRegType === 'new' ? $this->firstWorkingPlace : $this->currentWorkingPlace;
            $currentServiceId = $this->msoRegType === 'new' ? $this->service : $this->currentService;
            $currentRankId = $this->msoRegType === 'new' ? $this->serviceRank : $this->currentServiceRank;
            $currentAppointDate = $this->msoRegType === 'new' ? $this->firstAppointmentDate : $this->currentAppointmentDate;
            $currentAppointmentLetterNo = $this->msoRegType === 'new' ? $this->appointmentLetterNo : $this->currentAppointmentLetterNo;
            ...
            // Assign management assistant officer role
            $user->assignRole('management assistant');
            ...
            session()->flash('success', 'Management Assistant Officer created successfully! Default password: '. $password);
            $this->resetForm();
            $this->dispatch('scroll-top');
            ...
            DB::rollBack();
            session()->flash('error', 'Database error: Unable to save Management Assistant Officer data. Duplicate entry');
            $this->dispatch('scroll-top');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Management Assistant Officer creation error: ' . $e->getMessage(), [
    ...
    {
        return view('livewire.m-s-o.m-s-o-create');
    }
}
