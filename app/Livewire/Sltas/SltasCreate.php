...
class SltasCreate extends Component
{
    ...
    // Current Appointment
    // -------------------------
    public $sltasRegType = 'existing', $currentAppointmentDate, $currentAppointmentLetterNo, $currentService, $currentServiceRank;
    ...
    protected function rulesForCurrentStep()
    {
        switch ($this->step) {
            ...
            case 5:
                return [
                    'sltasRegType' => 'required|string|in:new,existing',
    ...
    public function mount()
    {
        ...
        $this->servicesOption = Service::where('service_id', 'SER003')->active()->get();
    ...
    // -------------------------
    public function updatedSltasRegType($value)
    {
        if ($value === 'new') {
            ...
            // Reset current appointment fields for existing Teacher Advisor officer
            $this->reset([
                'currentAppointmentDate',
    ...
    public function save()
    {
        ...
        try {
            ...
            $currentOfficeLevelID = $this->sltasRegType === 'new' ? $this->firstOfficeLevel : $this->currentOfficeLevel;
            $currentWorkplaceId = $this->sltasRegType === 'new' ? $this->firstWorkingPlace : $this->currentWorkingPlace;
            $currentServiceId = $this->sltasRegType === 'new' ? $this->service : $this->currentService;
            $currentRankId = $this->sltasRegType === 'new' ? $this->serviceRank : $this->currentServiceRank;
            $currentAppointDate = $this->sltasRegType === 'new' ? $this->firstAppointmentDate : $this->currentAppointmentDate;
            $currentAppointmentLetterNo = $this->sltasRegType === 'new' ? $this->appointmentLetterNo : $this->currentAppointmentLetterNo;
            ...
            // Assign Teacher Advisor role
            $user->assignRole('Teacher Advisor');
            ...
            session()->flash('success', 'Teacher Advisor created successfully! Default password: ' . $password);
            $this->resetForm();
            $this->dispatch('scroll-top');
            ...
            DB::rollBack();
            session()->flash('error', 'Database error: Unable to save Teacher Advisor data. Duplicate entry');
            $this->dispatch('scroll-top');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Teacher Advisor creation error: ' . $e->getMessage(), [
