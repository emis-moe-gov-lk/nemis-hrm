...
class TeacherAnnualTransfer extends Component
{
    ...
    {
        $applications = TeacherTransferApplication::with('policy')
            ->where('employee_id', Auth::user()->people_id)
            ->orderBy('created_at', 'desc')
            ->get();

        $currentAnnualPolicy = TransferPolicy::active()
            ->where('service_id', 'SER001')
    ...
    public function statusBadge($status)
    {
        return match ($status) {
            ...
            'rejected' => ['color' => 'rose', 'label' => 'Rejected', 'bg' => 'bg-rose-50 dark:bg-rose-500/10', 'text' => 'text-rose-600', 'border' => 'border-rose-200 dark:border-rose-500/20', 'ring' => 'ring-rose-500/5'],
