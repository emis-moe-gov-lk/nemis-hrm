...
class SlasCreate extends Component
{
    ...
    // Current Appointment
    // -------------------------
    public $slasRegType = 'existing', $currentAppointmentDate, $currentAppointmentLetterNo, $currentService, $currentServiceRank;
    ...
    public $currentPositionOption = [], $positionOption = [];
    ...
    protected function rulesForCurrentStep()
    {
        switch ($this->step) {
            ...
            case 5:
                return [
                    'slasRegType' => 'required|string|in:new,existing',
    ...
    public function mount()
    {
        ...
        $this->servicesOption = Service::where('service_id', 'SER006')->active()->get();
    ...
    // -------------------------
    public function updatedSlasRegType($value)
    {
        if ($value === 'new') {
            ...
            // Reset current appointment fields for existing SLAS officer
            $this->reset([
                'currentAppointmentDate',
    ...
    public function save()
    {
        ...
        try {
            ...
            $currentOfficeLevelID = $this->slasRegType === 'new' ? $this->firstOfficeLevel : $this->currentOfficeLevel;
            $currentWorkplaceId = $this->slasRegType === 'new' ? $this->firstWorkingPlace : $this->currentWorkingPlace;
            $currentServiceId = $this->slasRegType === 'new' ? $this->service : $this->currentService;
            $currentRankId = $this->slasRegType === 'new' ? $this->serviceRank : $this->currentServiceRank;
            $currentAppointDate = $this->slasRegType === 'new' ? $this->firstAppointmentDate : $this->currentAppointmentDate;
            $currentAppointmentLetterNo = $this->slasRegType === 'new' ? $this->appointmentLetterNo : $this->currentAppointmentLetterNo;
            ...
            // Assign SLAS role
            $user->assignRole('Administrative Service');
            ...
            session()->flash('success', 'Administrative Service (SLAS) Officer created successfully! Default password: ' . $password);
            $this->resetForm();
            $this->dispatch('scroll-top');
            ...
            DB::rollBack();
            session()->flash('error', 'Database error: Unable to save Administrative Service (SLAS) Officer data. Duplicate entry');
            $this->dispatch('scroll-top');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Administrative Service (SLAS) Officer creation error: ' . $e->getMessage(), [
    ...
    {
        return view('livewire.slas.slas-create');
    }
}
