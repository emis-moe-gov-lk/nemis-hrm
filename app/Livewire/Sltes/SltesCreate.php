...
class SltesCreate extends Component
{
    ...
    // Current Appointment
    // -------------------------
    public $sltesRegType = 'existing', $currentAppointmentDate, $currentAppointmentLetterNo, $currentService, $currentServiceRank;
    ...
    protected function rulesForCurrentStep()
    {
        switch ($this->step) {
            ...
            case 5:
                return [
                    'sltesRegType' => 'required|string|in:new,existing',
    ...
    public function mount()
    {
        ...
        $this->servicesOption = Service::where('service_id', 'SER002')->active()->get();
    ...
    // -------------------------
    public function updatedSltesRegType($value)
    {
        if ($value === 'new') {
            ...
            // Reset current appointment fields for existing Teacher Educator officer
            $this->reset([
                'currentAppointmentDate',
    ...
    public function save()
    {
        ...
        try {
            ...
            $currentOfficeLevelID = $this->sltesRegType === 'new' ? $this->firstOfficeLevel : $this->currentOfficeLevel;
            $currentWorkplaceId = $this->sltesRegType === 'new' ? $this->firstWorkingPlace : $this->currentWorkingPlace;
            $currentServiceId = $this->sltesRegType === 'new' ? $this->service : $this->currentService;
            $currentRankId = $this->sltesRegType === 'new' ? $this->serviceRank : $this->currentServiceRank;
            $currentAppointDate = $this->sltesRegType === 'new' ? $this->firstAppointmentDate : $this->currentAppointmentDate;
            $currentAppointmentLetterNo = $this->sltesRegType === 'new' ? $this->appointmentLetterNo : $this->currentAppointmentLetterNo;
            ...
            // Assign Teacher Educator role
            $user->assignRole('Teacher Educator');
            ...
            session()->flash('success', 'Teacher Educator created successfully! Default password: ' . $password);
            $this->resetForm();
            $this->dispatch('scroll-top');
            ...
            DB::rollBack();
            session()->flash('error', 'Database error: Unable to save Teacher Educator data. Duplicate entry');
            $this->dispatch('scroll-top');
        } catch (\Throwable $e) {
            DB::rollBack();
            Log::error('Teacher Educator creation error: ' . $e->getMessage(), [
